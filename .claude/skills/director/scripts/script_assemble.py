...
def main():
    ...
    # Always emit the position -> source-capture-idx map. validate_scenario's
    # legacy fallback matches step N-of-path to response N-of-path, which
    # silently verifies against the WRONG response the moment the model keeps
    # a later duplicate but drops the earlier one (occurrence counting shifts
    # by one). The map makes the correspondence explicit; synthetic steps map
    # to null.
    cap_map = {str(pos): (None if s.get("synthetic") else s["idx"])
               for pos, s in enumerate(kept)}
    map_path = args.out.rsplit(".", 1)[0] + ".capture_map.json"
    with open(map_path, "w", encoding="utf-8") as f:
        json.dump({"position_to_capture_idx": cap_map}, f, indent=2)
    print(f"[ok] wrote {map_path} — pass to validate_scenario.py --capture-map "
          f"for exact response matching")
