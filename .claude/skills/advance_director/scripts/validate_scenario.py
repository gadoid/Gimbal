...
def main():
    ...
    ap.add_argument("--capture-map", default=None,
                    help="*.capture_map.json sidecar from script_assemble.py mapping final "
                         "step position -> source capture idx; enables EXACT response "
                         "matching (synthetic steps auto-skipped) instead of legacy "
                         "per-path occurrence counting")
    args = ap.parse_args()
    ...
    if args.capture:
        ...
        resp_by_idx = {}
        for li, line in enumerate(open(args.capture, encoding="utf-8")):
            line = line.strip()
            if not line:
                continue
            try:
                ...
                continue
            body = parse_body((r.get("response") or {}).get("body"))
            resp_by_path[r.get("path")].append(body)
            resp_by_idx[li] = body
        # Exact matching via the assemble-stage sidecar (position -> capture
        # idx). Without it we fall back to legacy occurrence counting, which
        # goes wrong as soon as the kept-vs-dropped occurrences of a repeated
        # path don't line up 1:1 with capture order (e.g. the model dropped
        # the FIRST orderDetail and kept the SECOND: occurrence counting then
        # verifies the kept step against the first response — wrong audit_id).
        cap_map = None
        if args.capture_map:
            cm = json.load(open(args.capture_map, encoding="utf-8"))
            cap_map = {int(k): v for k, v in cm.get("position_to_capture_idx", {}).items()}
        ...
        for i, s in enumerate(steps):
            ...
            if cap_map is not None:
                src = cap_map.get(i)
                if src is None:      # synthetic — no backing record by design
                    continue
                body = resp_by_idx.get(src)
            else:
