...
name: director_tmp
description: >-
  Temporary fork of gimbal-traffic-to-scenario for incremental optimization
  experiments. Convert captured API traffic (Prism/mitmproxy ndjson, where
  each line is {ts, method, host, path, headers, body, response}) into a
  schema-valid, data-driven GIMBAL test scenario — WITHOUT depending on a
  golden example scenario for runtime judgment. Use this whenever the user
  wants to turn a real business flow / traffic capture / ndjson /
  FlowSession dump into a GIMBAL scenario or test case, "distill" or
  "denoise" captured requests into an ordered API chain, generate
  extract/assign context-passing between steps, or mentions GIMBAL scenario
  generation, Director skill, 流量转用例, 抓包生成测试, ndjson 蒸馏,
  剪辑链, 发版链, or 顺序接口数据驱动用例. Trigger even if they only provide
  the ndjson and a scenario scaffold without saying "skill".
---
...
## Windows path quoting (read this if you are on Windows)

The example commands below use POSIX-style backslashes (`\`) for line
continuation. On Windows, depending on which shell you run them in, those
need adjustment:

- **Git Bash / WSL**: copy commands verbatim. Line continuations and
  `--flag "value with spaces"` work as written.
- **PowerShell**: replace trailing `\` with `` ` `` (backtick). Example:
  ```powershell
  python scripts/analyze_flow.py capture.ndjson `
      ...
      --out flow.json
  ```
  Quote any path containing spaces: `--scaffold "C:\my dir\scaffold.json"`.
- **cmd.exe**: prefer Git Bash or PowerShell — `cmd` doesn't have a
  reliable line-continuation story for long Python invocations.

Common Windows-specific pitfalls:

1. **`${var.x}` body strings** in `--request-body '{...}'` MUST be
   single-quoted in bash but DOUBLE-quoted-then-escaped in PowerShell
   (`"--request-body '{ \"bl_no\": \"GIMBAL_TEST_35\" }'"`), because
   `$` is a PowerShell variable sigil. If you see the literal value
   `GIMBAL_TEST_35` end up as empty in the captured script.json,
   PowerShell ate your `${...}` placeholder.
2. **Forward vs back slashes**: the scripts accept either, but PowerShell's
   `ConvertFrom-Json` (used by some users when eyeballing) requires
   `Get-Content -Raw` and backslashes in paths inside JSON strings.
3. **UTF-8 BOM**: capture files written by Windows tools may carry a BOM.
   The scripts strip it, but if a downstream grep shows odd characters,
   re-save the capture as UTF-8 without BOM.
