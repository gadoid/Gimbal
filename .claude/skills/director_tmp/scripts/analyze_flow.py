...
def resolve_path(body, jsonpath):
    """Resolve $.response_body.a.b[0].c against a parsed body dict.
    Returns the leaf value if every segment walks cleanly, else None.
    Designed for the producer-side sanity check added in change 1B: we trust the
    path only if it actually walks to a value at the producer's response body.
    """
    if body is None:
        return None
    expr = jsonpath.replace("$.response_body", "", 1) if jsonpath.startswith("$.response_body") else jsonpath
    cur = body
    # Walk ".a.b[0].c" -> ("a","b"), ("", "0")
    for key, ix in re.findall(r"\.([A-Za-z0-9_]+)|\[(\d+)\]", expr):
        try:
            if key:
...
def verify_lineage_edges(lineage, parsed_by_idx, log=print):
    """Change 1B: lineage-sanity check.

    The producer-discovery loop records the first JSONPath where an id-shaped
    value appeared in any response, but it does NOT verify that the same path
    actually walks to that value at the recorded producer step (a different
    response at the same path can contain a different id, or the path can be
    invalid for that specific body). This pass re-parses each producer's
    response body and drops any edge whose producer_path does not resolve to
    the expected value. Surviving edges get a `verified: true` flag so
    downstream stages can trust them.

    Why at analyze_flow.py time, not later: this is where the producer/consumer
    facts first exist. Doing it here means every downstream consumer
    (script_init, validate_scenario) sees only verified edges.
    """
    verified, dropped = [], []
    for edge in lineage:
        prod_body = parsed_by_idx.get(edge["producer_idx"], {}).get("resp")
        got = resolve_path(prod_body, edge["producer_path"])
        if got is None or str(got) != edge["value"]:
            dropped.append(edge)
        else:
            edge = {**edge, "verified": True}
            verified.append(edge)
    if dropped:
        log(f"[change-1B] dropped {len(dropped)} unverified lineage edge(s) "
            f"(producer_path did not walk to the expected value):")
        # group by producer for compact reporting
        by_prod = defaultdict(list)
        for e in dropped:
            by_prod[(e["producer_idx"], e["producer_path"])].append(e)
        for (prod_idx, prod_path), edges in sorted(by_prod.items()):
            sample = edges[0]
            log(f"  - producer step idx={prod_idx} path={prod_path!r} "
                f"-> value {sample['value']!r}: {len(edges)} consumer edge(s) dropped "
                f"(field={sample['field']})")
    log(f"[change-1B] verified {len(verified)} / {len(lineage)} lineage edges")
    return verified
...
def main():
    ...
    # Change 1B: drop lineage edges whose producer_path does not actually walk
    # to the expected value at the recorded producer step. This MUST run before
    # any downstream code reads `lineage` or `producer_emits` (the keep/drop
    # classification below uses producer_emits to decide whether a record
    # survives, so we'd otherwise attribute producer-ness to a step whose body
    # doesn't really emit the id). Re-derive producer_emits from the verified
    # lineage so keep/drop logic reflects ground truth.
    parsed_by_idx = {p["idx"]: p for p in parsed}
    lineage = verify_lineage_edges(lineage, parsed_by_idx)
    producer_emits = defaultdict(list)
    consumer_needs = defaultdict(list)
    for edge in lineage:
        producer_emits[edge["producer_idx"]].append(edge)
        consumer_needs[edge["consumer_idx"]].append(edge)
