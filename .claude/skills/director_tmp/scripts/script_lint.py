...
def main():
    ...
    ap.add_argument("--out", default=None,
                    help="optional path to write the lint result as JSON "
                         "{violations, warnings, kept_steps, extract_vars} so "
                         "downstream tools (e.g. script_assemble.py report "
                         "generator) can read it without re-running lint.")
    args = ap.parse_args()
    ...
    if args.out:
        payload = {
            "script": args.script_json,
            "kept_steps": len(steps),
            "extract_vars": sorted(extract_vars_seen),
