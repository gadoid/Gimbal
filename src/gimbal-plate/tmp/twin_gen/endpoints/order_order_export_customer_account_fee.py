"""fin.order_order.export_customer_account_fee —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): (无)
溯源统计: 无zh=1, derived=1 | fe_high=0, enum=0
"""
...
ORDER_EXPORT_CUSTOMER_ACCOUNT_FEE: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.export_customer_account_fee',
    system='fin',
    service='fin-service',
    ...
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='split_type', path=f'$.split_type', type='string', state='form', ui_kind='text', required=True),
