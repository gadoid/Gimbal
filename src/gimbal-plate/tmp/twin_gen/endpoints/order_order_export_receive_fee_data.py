"""fin.order_order.export_receive_fee_data —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_EXPORT_RECEIVE_FEE_DATA: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.export_receive_fee_data',
    system='fin',
    service='fin-service',
    name='Order.exportReceiveFeeData',
    description='Order.exportReceiveFeeData' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/exportReceiveFeeData',
        headers={},
        consumes=[],
        produces=[],
    ),
