...
needs_capture(首跑经 gimbal 执行回填): service_project, customer_id, order_no, status, order_self_id, page_no, page_size, sort_field, sort_order
溯源统计: frontend=5, builtin=4, lang=1, column=1 | fe_high=6, enum=1
"""
...
ORDER_SELF_FEE_OTHER_ORDER_FEE_PAGE: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='service_project', path=f'$.service_project', type='string', state='carry', ui_kind='select', required=True, description='服务项目'),  # zh_ambiguous, needs_capture:value_source
