"""fin.order_entrust.order_update_atd —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_ENTRUST_ORDER_UPDATE_ATD: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_entrust.order_update_atd',
    system='fin',
    service='fin-service',
    name='操作目的',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/orderEntrust/orderUpdateAtd',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='atd', path=f'$.atd', type='integer', state='form', required=True, default='0', description='实际开航日'),
