"""fin.order_self_fee.batch_fee_add_error_bl_no —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): order_self_fee_id
溯源统计: column=15, 无zh=9 | fe_high=0, enum=0
"""
...
ORDER_SELF_FEE_BATCH_FEE_ADD_ERROR_BL_NO: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_self_fee.batch_fee_add_error_bl_no',
    system='fin',
    service='fin-service',
    name='OrderSelfFee.batchFeeAddErrorBlNo',
    description='OrderSelfFee.batchFeeAddErrorBlNo' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/orderSelfFee/batchFeeAddErrorBlNo',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='order_id', path=f'$.order_id', type='integer', state='carry', ui_kind='number', description='订单id'),
            DeclarationEntry(name='order_self_id', path=f'$.order_self_id', type='integer', state='carry', ui_kind='number', description='子订单id'),
            DeclarationEntry(name='service_project', path=f'$.service_project', type='string', state='carry', ui_kind='text', description='服务项目'),
