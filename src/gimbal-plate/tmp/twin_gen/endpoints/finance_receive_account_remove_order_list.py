"""fin.receive_account.remove_order_list —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): page_no, page_size, sort_field, sort_order
溯源统计: builtin=4, 无zh=2 | fe_high=0, enum=1
"""
...
RECEIVE_ACCOUNT_REMOVE_ORDER_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.receive_account.remove_order_list',
    system='fin',
    service='fin-service',
    name='ReceiveAccount.removeOrderList',
    description='ReceiveAccount.removeOrderList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/receiveAccount/removeOrderList',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='receive_order_ids', path=f'$.receive_order_ids', type='string', state='form', ui_kind='text'),
