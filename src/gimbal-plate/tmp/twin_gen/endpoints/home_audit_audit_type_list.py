"""fin.audit.audit_type_list —— 孪生生成器产物(请求面;行为面归场景用例)。
...
AUDIT_AUDIT_TYPE_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.audit.audit_type_list',
    system='fin',
    service='fin-service',
    name='Audit.auditTypeList',
    description='Audit.auditTypeList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/home/audit/auditTypeList',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='active_tab', path=f'$.active_tab', type='string', state='form', ui_kind='text', default='examine_wait'),
