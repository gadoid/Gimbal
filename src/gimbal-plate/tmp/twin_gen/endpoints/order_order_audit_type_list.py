"""fin.order_order.audit_type_list —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_AUDIT_TYPE_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.audit_type_list',
    system='fin',
    service='fin-service',
    name='Order.auditTypeList',
    description='Order.auditTypeList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/auditTypeList',
        headers={},
        consumes=[],
        produces=[],
    ),
