...
needs_capture(首跑经 gimbal 执行回填): dlgt_mobile_no, id_no_type, opr_person_type, id_no, company_no, status
"""
...
PRODUCTION_FINANCE_SYNC_COMPANY_USER: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='opr_person_type', path=f'$.opr_person_type', type='integer', state='carry', required=True, description='账号类型', enum=['1', '2', '3']),  # needs_capture:enum_required
            ...
            DeclarationEntry(name='company_name', path=f'$.company_name', type='string', state='carry', description='客户名称'),
        ],
    ),
    responses={
        200: ResponseSpec(
