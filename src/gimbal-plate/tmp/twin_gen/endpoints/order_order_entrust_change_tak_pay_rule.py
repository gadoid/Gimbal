"""fin.order_entrust.change_tak_pay_rule —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_ENTRUST_CHANGE_TAK_PAY_RULE: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_entrust.change_tak_pay_rule',
    system='fin',
    service='fin-service',
    name='操作目的',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/orderEntrust/changeTakPayRule',
        headers={},
        consumes=[],
        produces=[],
    ),
