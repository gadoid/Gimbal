...
needs_capture(首跑经 gimbal 执行回填): king_dee_account_number
"""
...
MAIN_MAIN_EDIT: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='main_name_cn', path=f'$.main_name_cn', type='string', state='form', required=True),
            DeclarationEntry(name='main_name_en', path=f'$.main_name_en', type='string', state='form', required=True, description='主体英文名称'),
            DeclarationEntry(name='main_name', path=f'$.main_name', type='string', state='form', required=True, description='费用主体名称'),
            DeclarationEntry(name='status', path=f'$.status', type='integer', state='form', required=True, default='0', description='状态 1未通知 2已通知'),
            DeclarationEntry(name='curator_user_id', path=f'$.curator_user_id', type='string', state='form', required=True, description='主体负责人'),
            DeclarationEntry(name='is_generate', path=f'$.is_generate', type='string', state='form', required=True, description='是否生成凭证 1是，0否'),
            DeclarationEntry(name='identity', path=f'$.identity', type='string', state='form', required=True, description='身份'),
            ...
            DeclarationEntry(name='is_sync', path=f'$.is_sync', type='string', state='form', required=True, description='是否同步 1是，0否'),
            DeclarationEntry(name='simplified_code', path=f'$.simplified_code', type='string', state='form', required=True, description='简码'),
            DeclarationEntry(name='tax_number', path=f'$.tax_number', type='string', state='form', required=True, description='税号'),
            DeclarationEntry(name='address_cn', path=f'$.address_cn', type='string', state='form', description='中文详细地址'),
            DeclarationEntry(name='address_en', path=f'$.address_en', type='string', state='form', description='英文详细地址'),
            DeclarationEntry(name='remark', path=f'$.remark', type='string', state='form', description='备注'),
            DeclarationEntry(name='king_dee_account_number', path=f'$.king_dee_account_number', type='string', state='carry', required=True, description='金蝶账簿编号'),  # needs_capture:value_source
