"""fin.order_order.batch_edit_settle_object —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): order_supplier_id, currency, service_project, put_settle_object_id, pay_settle_object_id, symbol, order_id, settle_object_id, unique_id, customer_id, customer_address_cn, customer_contact_name, customer_contact_phone, main_sort, pol_cn, pol_country, pol_country_cn, pod_cn, pot, pot_cn, del, del_cn, country_name, ocean_type, airline_type, terms_payment, terms_transport, terms_shipment, pay_type, customer_order_sn, packer, volume_desc, sea_trans_currency, shipper, consignee, notifier, ship_mark, commodity, notes, discount_rule, discount_ratio, discount_currency, fee_miss_name, effective_by, audit_type, customer_put_date_desc
溯源统计: column=192, lang=17, 无zh=14, header=1 | fe_high=26, enum=1
"""
...
ORDER_BATCH_EDIT_SETTLE_OBJECT: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.batch_edit_settle_object',
    system='fin',
    service='fin-service',
    name='操作',
    ...
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='atd', path=f'$.atd', type='integer', state='carry', ui_kind='number', default='0', description='实际开航日'),
            DeclarationEntry(name='finance_date', path=f'$.finance_date', type='integer', state='carry', ui_kind='number', default='0', description='财务日期'),
            DeclarationEntry(name='period_rule', path=f'$.period_rule', type='integer', state='carry', ui_kind='number', default='0', description='截转规则'),
            ...
            DeclarationEntry(name='service_item', path=f'$.service_item', type='string', state='form', ui_kind='text', description='服务项目'),
            DeclarationEntry(name='pay_date', path=f'$.pay_date', type='integer', state='carry', ui_kind='number', default='0', description='供应商应付日'),
            DeclarationEntry(name='take_date', path=f'$.take_date', type='integer', state='carry', ui_kind='number', default='0', description='客户应收日'),
            ...
            DeclarationEntry(name='order_supplier_id', path=f'$.order_supplier_id', type='integer', state='carry', ui_kind='number'),  # needs_capture:value_source
            DeclarationEntry(name='modify_type', path=f'$.modify_type', type='string', state='form', ui_kind='text', description='修改维度'),
            DeclarationEntry(name='currency', path=f'$.currency', type='string', state='form', ui_kind='text', description='币制'),  # zh_ambiguous
            ...
            DeclarationEntry(name='old_put_settle_object_id', path=f'$.old_put_settle_object_id', type='string', state='form', ui_kind='text', description='应收结算对象'),
            ...
            DeclarationEntry(name='old_pay_settle_object_id', path=f'$.old_pay_settle_object_id', type='string', state='form', ui_kind='text', description='应付结算对象'),
            ...
            DeclarationEntry(name='checkData', path=f'$.checkData', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='settle_object', path=f'$.settle_object', type='string', state='form', ui_kind='text', description='结算对象'),
            DeclarationEntry(name='settle_object_id', path=f'$.settle_object_id', type='integer', state='form', ui_kind='number', default='0', description='结算对象ID'),  # zh_ambiguous
            DeclarationEntry(name='voucher_fee_real_ids', path=f'$.voucher_fee_real_ids', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='sorce', path=f'$.sorce', type='string', state='form', ui_kind='text', description='对象来源  main 主体  supplier 供应商  customer 客户'),
            DeclarationEntry(name='relation_id', path=f'$.relation_id', type='integer', state='form', ui_kind='number'),
            DeclarationEntry(name='unique_id', path=f'$.unique_id', type='string', state='form', ui_kind='text', description='唯一标记  同源费用标记一致'),  # zh_ambiguous
            ...
            DeclarationEntry(name='self_support', path=f'$.self_support', type='string', state='form', ui_kind='text', default='0', description='是否为自营供应商 0否 1是'),
            DeclarationEntry(name='unique', path=f'$.unique', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='settle_object_type', path=f'$.settle_object_type', type='string', state='form', ui_kind='text', description='结算对象类型 customer客户/supplier供应商'),
