"""fin.pay_account.pay_confirm_list —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): page_no, page_size, sort_field, sort_order
溯源统计: builtin=4, 无zh=3, column=1 | fe_high=0, enum=1
"""
...
PAY_ACCOUNT_PAY_CONFIRM_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.pay_account.pay_confirm_list',
    system='fin',
    service='fin-service',
    name='PayAccount.payConfirmList',
    description='PayAccount.payConfirmList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/payAccount/payConfirmList',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='confirm_type', path=f'$.confirm_type', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='quick_status', path=f'$.quick_status', type='string', state='form', ui_kind='text'),
            ...
            DeclarationEntry(name='order_ids', path=f'$.order_ids', type='string', state='form', ui_kind='text'),
