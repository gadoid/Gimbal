"""fin.post.post_page —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): post_id, status, sort_field, sort_order
溯源统计: column=4, builtin=4 | fe_high=1, enum=1
"""
...
POST_POST_PAGE: Final[EndpointSpec] = EndpointSpec(
    id='fin.post.post_page',
    system='fin',
    service='fin-service',
    name='Post.postPage',
    description='Post.postPage' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/home/post/postPage',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='post_id', path=f'$.post_id', type='integer', state='carry', ui_kind='number', description='岗位ID'),  # needs_capture:value_source
            DeclarationEntry(name='post_name', path=f'$.post_name', type='string', state='form', ui_kind='text', description='岗位名称'),
            DeclarationEntry(name='post_code', path=f'$.post_code', type='string', state='form', ui_kind='text', description='岗位编码'),
