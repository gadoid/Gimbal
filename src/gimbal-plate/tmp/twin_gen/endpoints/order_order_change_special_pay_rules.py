"""fin.order_order.change_special_pay_rules —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): flow_type
溯源统计: column=6, 无zh=3, derived=1 | fe_high=0, enum=1
"""
...
ORDER_CHANGE_SPECIAL_PAY_RULES: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.change_special_pay_rules',
    system='fin',
    service='fin-service',
    name='操作',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/changeSpecialPayRules',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='title', path=f'$.title', type='string', state='form', ui_kind='text', description='消息标题'),
            DeclarationEntry(name='code', path=f'$.code', type='string', state='form', ui_kind='text', description='编码'),
            DeclarationEntry(name='msgs', path=f'$.msgs', type='string', state='form', ui_kind='text'),
