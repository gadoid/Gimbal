"""fin.ship_company.ship_company_list —— 孪生生成器产物(请求面;行为面归场景用例)。
...
SHIP_COMPANY_SHIP_COMPANY_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.ship_company.ship_company_list',
    system='fin',
    service='fin-service',
    name='ShipCompany.shipCompanyList',
    description='ShipCompany.shipCompanyList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/home/shipCompany/shipCompanyList',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='query_string', path=f'$.query_string', type='string', state='form', ui_kind='text'),
