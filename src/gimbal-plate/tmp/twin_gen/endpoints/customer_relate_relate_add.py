...
RELATE_RELATE_ADD: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='supplier_id', path=f'$.supplier_id', type='integer', state='form', required=True, default='0', description='变更账期供应商ID'),
            DeclarationEntry(name='period_rule', path=f'$.period_rule', type='string', state='form', required=True, default='1', description='废弃 125日后订单截转下月计算账期 2按ATD本月计算账期', enum=['1', '2']),
            DeclarationEntry(name='pay_time_limit', path=f'$.pay_time_limit', type='integer', state='form', description='付款时效'),
            DeclarationEntry(name='is_independent_email', path=f'$.is_independent_email', type='integer', state='form', required=True, default='0', description='是否独立对接供应商邮箱：0否 1是', enum=['0', '1']),
            DeclarationEntry(name='period_date_start', path=f'$.period_date_start', type='integer', state='carry', description='生效时间'),
            DeclarationEntry(name='period_date_end', path=f'$.period_date_end', type='integer', state='carry', description='失效时间'),
