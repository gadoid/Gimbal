...
needs_capture(首跑经 gimbal 执行回填): customer_id, main_id, pay_settle_object_id, symbol
溯源统计: lang=3, column=2, builtin=2, 无zh=1 | fe_high=0, enum=0
"""
...
ACCOUNT_FEE_FINANCE_PAY_EXPORT: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='pay_settle_object_id', path=f'$.pay_settle_object_id', type='integer', state='form', ui_kind='number', required=True, default='0', description='应付结算对象'),  # zh_ambiguous
