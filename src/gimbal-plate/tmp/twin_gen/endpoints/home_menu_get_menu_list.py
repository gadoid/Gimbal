"""fin.menu.get_menu_list —— 孪生生成器产物(请求面;行为面归场景用例)。
...
MENU_GET_MENU_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.menu.get_menu_list',
    system='fin',
    service='fin-service',
    name='Menu.getMenuList',
    description='Menu.getMenuList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/home/menu/getMenuList',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='menu_id', path=f'$.menu_id', type='integer', state='carry', ui_kind='number', description='菜单ID'),
