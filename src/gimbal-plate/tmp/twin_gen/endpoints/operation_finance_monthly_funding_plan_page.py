"""fin.operation_finance.monthly_funding_plan_page —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): type, page_no, page_size, sort_field, sort_order
溯源统计: builtin=4, column=2, 无zh=1 | fe_high=0, enum=2
"""
...
FINANCE_MONTHLY_FUNDING_PLAN_PAGE: Final[EndpointSpec] = EndpointSpec(
    id='fin.operation_finance.monthly_funding_plan_page',
    system='fin',
    service='fin-service',
    name='Finance.monthlyFundingPlanPage',
    description='Finance.monthlyFundingPlanPage' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/operation/finance/monthlyFundingPlanPage',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='year', path=f'$.year', type='string', state='form', ui_kind='text', required=True, description='年份'),
            DeclarationEntry(name='month', path=f'$.month', type='string', state='form', ui_kind='text', required=True),
