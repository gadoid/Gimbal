"""fin.receive_writeoff.writeoff_export —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): receive_invoice_batch_no, create_time, link_audit_status, customer_id, main_id, currency, create_id, pay_settle_object_id, receive_writeoff_no, writeoff_status, pay_invoice_batch_no, receive_invoice_apply_no, cancel_status, put_settle_object_id, invoice_status, invoice_no
溯源统计: frontend=33, derived=6, column=5, lang=1, 无zh=1 | fe_high=34, enum=0
"""
...
RECEIVE_WRITEOFF_WRITEOFF_EXPORT: Final[EndpointSpec] = EndpointSpec(
    id='fin.receive_writeoff.writeoff_export',
    system='fin',
    service='fin-service',
    name='ReceiveWriteoff.writeoffExport',
    description='ReceiveWriteoff.writeoffExport' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/receiveWriteoff/writeoffExport',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='receive_writeoff_no', path=f'$.receive_writeoff_no', type='string', state='carry', ui_kind='text', description='关联对向核销记录ID'),  # needs_capture:value_source
            ...
            DeclarationEntry(name='receive_settle_object_id', path=f'$.receive_settle_object_id', type='integer', state='form', ui_kind='text', default='0', description='应收结算对象'),
            DeclarationEntry(name='pay_writeoff_no', path=f'$.pay_writeoff_no', type='string', state='form', ui_kind='text', description='关联对向核销记录ID'),
            ...
            DeclarationEntry(name='receive_writeoff_id', path=f'$.receive_writeoff_id', type='integer', state='form', ui_kind='number', default='0', description='核销ID'),
