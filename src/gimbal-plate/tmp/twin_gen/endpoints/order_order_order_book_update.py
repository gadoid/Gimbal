"""fin.order_order.order_book_update —— 孪生生成器产物(请求面;行为面归场景用例)。
...
溯源统计: column=190, 无zh=2, lang=1 | fe_high=0, enum=0
"""
...
ORDER_ORDER_BOOK_UPDATE: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.order_book_update',
    system='fin',
    service='fin-service',
    name='Order.orderBookUpdate',
    description='Order.orderBookUpdate' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/orderBookUpdate',
        headers={},
        consumes=[],
        produces=[],
    ),
