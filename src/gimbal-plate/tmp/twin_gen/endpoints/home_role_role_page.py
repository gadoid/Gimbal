"""fin.role.role_page —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ROLE_ROLE_PAGE: Final[EndpointSpec] = EndpointSpec(
    id='fin.role.role_page',
    system='fin',
    service='fin-service',
    name='Role.rolePage',
    description='Role.rolePage' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/home/role/rolePage',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='role_id', path=f'$.role_id', type='integer', state='carry', ui_kind='number', description='角色ID'),
