"""fin.order_fee.un_lock_account_fee_list —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_FEE_UN_LOCK_ACCOUNT_FEE_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_fee.un_lock_account_fee_list',
    system='fin',
    service='fin-service',
    name='OrderFee.unLockAccountFeeList',
    description='OrderFee.unLockAccountFeeList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/orderFee/unLockAccountFeeList',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='audit_id', path=f'$.audit_id', type='integer', state='form', ui_kind='number'),
