"""fin.order_order.batch_pay_settle_object_edit —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_BATCH_PAY_SETTLE_OBJECT_EDIT: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.batch_pay_settle_object_edit',
    system='fin',
    service='fin-service',
    name='Order.batchPaySettleObjectEdit',
    description='Order.batchPaySettleObjectEdit' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/batchPaySettleObjectEdit',
        headers={},
        consumes=[],
        produces=[],
    ),
