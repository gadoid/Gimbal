"""fin.pay_invoice.invoice_list —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): pay_settle_object_id, pay_invoice_apply_no, pay_invoice_batch_no, create_time, cancel_status, link_audit_status, customer_id, main_id, currency, invoice_status, invoice_no, writeoff_status, create_id, invoice_number, invoice_type, invoice_date, receive_invoice_apply_no, cancel_time, receive_invoice_batch_no, put_settle_object_id, bl_no, page_no, page_size, sort_field, sort_order
溯源统计: frontend=22, derived=10, lang=6, column=4, 无zh=4, builtin=4, rule=2 | fe_high=29, enum=1
"""
...
PAY_INVOICE_INVOICE_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.pay_invoice.invoice_list',
    system='fin',
    service='fin-service',
    name='PayInvoice.invoiceList',
    description='PayInvoice.invoiceList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/payInvoice/invoiceList',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='pay_settle_object_id', path=f'$.pay_settle_object_id', type='integer', state='form', ui_kind='select', required=True, default='0', description='应收结算对象ID'),  # zh_ambiguous
            ...
            DeclarationEntry(name='pay_invoice_apply_no', path=f'$.pay_invoice_apply_no', type='string', state='form', ui_kind='text', description='开票申请ID'),  # zh_ambiguous
            ...
            DeclarationEntry(name='pay_invoice_ids', path=f'$.pay_invoice_ids', type='string', state='form', ui_kind='text'),
