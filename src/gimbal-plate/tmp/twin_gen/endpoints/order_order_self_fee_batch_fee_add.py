"""fin.order_self_fee.batch_fee_add —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): order_self_id, service_project, order_self_fee_id
溯源统计: column=15, 无zh=11, lang=1 | fe_high=0, enum=1
"""
...
ORDER_SELF_FEE_BATCH_FEE_ADD: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_self_fee.batch_fee_add',
    system='fin',
    service='fin-service',
    name='操作',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/orderSelfFee/batchFeeAdd',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='put_amount', path=f'$.put_amount', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='pay_amount', path=f'$.pay_amount', type='string', state='form', ui_kind='text'),
