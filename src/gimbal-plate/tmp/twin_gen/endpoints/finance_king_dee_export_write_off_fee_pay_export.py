"""fin.king_dee_export.write_off_fee_pay_export —— 孪生生成器产物(请求面;行为面归场景用例)。
...
KING_DEE_EXPORT_WRITE_OFF_FEE_PAY_EXPORT: Final[EndpointSpec] = EndpointSpec(
    id='fin.king_dee_export.write_off_fee_pay_export',
    system='fin',
    service='fin-service',
    name='操作目的',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/kingDeeExport/writeOffFeePayExport',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='action', path=f'$.action', type='string', state='form', default='write_off', description='操作目的'),
