...
PAY_INVOICE_INVOICE_EDIT: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='pay_invoice_id', path=f'$.pay_invoice_id', type='string', state='form', required=True, default='0', description='发票id'),
            ...
            DeclarationEntry(name='buyer_identity', path=f'$.buyer_identity', type='string', state='carry', description='买方身份（可选，购买方须为系统内主体，费用主体随税号联动）', enum=['main']),
            ...
            DeclarationEntry(name='seller_identity', path=f'$.seller_identity', type='string', state='form', description='卖方身份（可选，传入时按身份精确匹配应付结算对象）', enum=['supplier', 'main']),
