"""fin.pay_invoice.invoice_edit —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): pay_invoice_id, invoice_number, invoice_type, invoice_amount, invoice_date, currency, invoice_exchange_rate, bl_no, fee_main_name, sorce, replace_invoice_original, cancel_by
溯源统计: column=30, rule=16, 无zh=3, lang=1 | fe_high=0, enum=4
"""
...
PAY_INVOICE_INVOICE_EDIT: Final[EndpointSpec] = EndpointSpec(
    id='fin.pay_invoice.invoice_edit',
    system='fin',
    service='fin-service',
    name='PayInvoice.invoiceEdit',
    description='PayInvoice.invoiceEdit' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/payInvoice/invoiceEdit',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='buyer_identity', path=f'$.buyer_identity', type='string', state='carry', ui_kind='select', description='买方身份（可选，购买方须为系统内主体，费用主体随税号联动）', enum=['main']),
            ...
            DeclarationEntry(name='seller_identity', path=f'$.seller_identity', type='string', state='form', ui_kind='select', description='卖方身份（可选，传入时按身份精确匹配应付结算对象）', enum=['supplier', 'main']),
