...
needs_capture(首跑经 gimbal 执行回填): tax_number, policy_types, status, customer_id, settlement_date, period_rule, create_by, create_time, update_by, update_time, create_id, allocation_no, policy_no, settle_type
溯源统计: column=21, 无zh=20, lang=19, rule=12, frontend=7, derived=3 | fe_high=61, enum=2
"""
...
CUSTOMER_CUSTOMER_ADD: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='customer_name', path=f'$.customer_name', type='string', state='carry', ui_kind='text', description='客户名称', value_source=ValueSource(view='customer_list', column='customer_name'), source_kind='lookup'),
            ...
            DeclarationEntry(name='tax_number', path=f'$.tax_number', type='string', state='carry', ui_kind='text', description='统一社会信用代码'),  # needs_capture:value_source
            ...
            DeclarationEntry(name='status', path=f'$.status', type='string', state='form', ui_kind='select', description='状态', enum=['1', '2']),  # zh_ambiguous
            DeclarationEntry(name='address_cn', path=f'$.address_cn', type='string', state='carry', ui_kind='text', description='中文详细地址'),
            ...
            DeclarationEntry(name='settlement_date', path=f'$.settlement_date', type='string', state='carry', ui_kind='text', default='10', description='客户结算日'),  # zh_ambiguous
            DeclarationEntry(name='period_rule', path=f'$.period_rule', type='string', state='carry', ui_kind='text', default='1', description='账期截转规则'),  # zh_ambiguous
            DeclarationEntry(name='create_by', path=f'$.create_by', type='string', state='carry', ui_kind='text', description='创建者'),  # zh_ambiguous
            DeclarationEntry(name='create_time', path=f'$.create_time', type='integer', state='carry', ui_kind='number', description='创建时间'),  # zh_ambiguous
            DeclarationEntry(name='update_by', path=f'$.update_by', type='string', state='carry', ui_kind='text', description='更新者'),  # zh_ambiguous
            DeclarationEntry(name='update_time', path=f'$.update_time', type='integer', state='carry', ui_kind='number', description='更新时间'),  # zh_ambiguous
            DeclarationEntry(name='person_in_charge_name', path=f'$.person_in_charge_name', type='string', state='carry', ui_kind='text', description='负责人名称'),
            DeclarationEntry(name='creation_method_name', path=f'$.creation_method_name', type='string', state='carry', ui_kind='text', description='创建方式名称'),
            ...
            DeclarationEntry(name='overdue_payment_name', path=f'$.overdue_payment_name', type='string', state='carry', ui_kind='text'),
            DeclarationEntry(name='ovdue_amount_usd', path=f'$.ovdue_amount_usd', type='string', state='carry', ui_kind='text'),
            DeclarationEntry(name='ovdue_amount_cny', path=f'$.ovdue_amount_cny', type='string', state='carry', ui_kind='text'),
            DeclarationEntry(name='ovdue_amount_total', path=f'$.ovdue_amount_total', type='string', state='carry', ui_kind='text'),
            ...
            DeclarationEntry(name='country_name_cn', path=f'$.country_name_cn', type='string', state='carry', ui_kind='text', description='中文国家'),
            ...
            DeclarationEntry(name='customer_product', path=f'$.customer_product', type='string', state='carry', ui_kind='text'),
            ...
            DeclarationEntry(name='creation_method', path=f'$.creation_method', type='string', state='form', ui_kind='select', description='创建方式'),
