"""fin.order_fee.real_amount_un_lock —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): order_id, customer_id, flow_type
溯源统计: column=8, 无zh=3, rule=1 | fe_high=0, enum=1
"""
...
ORDER_FEE_REAL_AMOUNT_UN_LOCK: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_fee.real_amount_un_lock',
    system='fin',
    service='fin-service',
    name='操作',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/orderFee/realAmountUnLock',
        headers={},
        consumes=[],
        produces=[],
    ),
