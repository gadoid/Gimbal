"""fin.order.order_edit —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-15T11:13:18+00:00
needs_capture(首跑经 gimbal 执行回填): customer_contact_name, operator_name, business_type, carrier, pol, pot, pod, del, terms_payment, terms_transport, pay_type, customer_order_sn, cargo_type, num, gross_weight, packer, terms_shipment, remark
"""
...
ORDER_ORDER_EDIT: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='trade_term', path=f'$.trade_term', type='string', state='form', required=True, description='成交方式'),
            DeclarationEntry(name='carrier', path=f'$.carrier', type='string', state='carry', required=True, description='船公司/承运人'),  # needs_capture:value_source
            ...
            DeclarationEntry(name='etd', path=f'$.etd', type='string', state='form', required=True, description='预计开航日'),
            DeclarationEntry(name='atd', path=f'$.atd', type='integer', state='form', default='0', description='实际开航日'),
            DeclarationEntry(name='ship_name', path=f'$.ship_name', type='string', state='form', required=True, description='船名'),
            DeclarationEntry(name='voy', path=f'$.voy', type='string', state='form', required=True, description='航次'),
            DeclarationEntry(name='pol', path=f'$.pol', type='string', state='carry', required=True, description='起运港'),  # needs_capture:value_source
            DeclarationEntry(name='pot', path=f'$.pot', type='string', state='carry', description='中转港'),  # needs_capture:value_source
            DeclarationEntry(name='pod', path=f'$.pod', type='string', state='carry', required=True, description='卸货港'),  # needs_capture:value_source
            DeclarationEntry(name='del', path=f'$.del', type='string', state='carry', required=True, description='目的地'),  # needs_capture:value_source
            ...
            DeclarationEntry(name='cargo_type', path=f'$.cargo_type', type='string', state='carry', description='货物类型'),  # needs_capture:value_source
            DeclarationEntry(name='num', path=f'$.num', type='integer', state='carry', required=True, default='0', description='件数'),  # needs_capture:value_source
            DeclarationEntry(name='gross_weight', path=f'$.gross_weight', type='number', state='carry', required=True, description='毛重'),  # needs_capture:value_source
            ...
            DeclarationEntry(name='financing_continue', path=f'$.financing_continue', type='string', state='form'),
