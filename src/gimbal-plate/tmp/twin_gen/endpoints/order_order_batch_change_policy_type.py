"""fin.order_order.batch_change_policy_type —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): (无)
溯源统计: column=3, 无zh=2, derived=1, lang=1 | fe_high=0, enum=1
"""
...
ORDER_BATCH_CHANGE_POLICY_TYPE: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.batch_change_policy_type',
    system='fin',
    service='fin-service',
    name='操作',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/batchChangePolicyType',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='order_list', path=f'$.order_list', type='string', state='form', ui_kind='text'),
