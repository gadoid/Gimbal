"""fin.audit.audit_note_add —— 孪生生成器产物(请求面;行为面归场景用例)。
...
溯源统计: column=21, 无zh=2 | fe_high=0, enum=0
"""
...
AUDIT_AUDIT_NOTE_ADD: Final[EndpointSpec] = EndpointSpec(
    id='fin.audit.audit_note_add',
    system='fin',
    service='fin-service',
    name='Audit.auditNoteAdd',
    description='Audit.auditNoteAdd' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/home/audit/auditNoteAdd',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='audit_id', path=f'$.audit_id', type='integer', state='form', ui_kind='number', required=True, default='0', description='审批ID'),
            DeclarationEntry(name='audit_user_note', path=f'$.audit_user_note', type='string', state='form', ui_kind='text', required=True, description='备注'),
