"""fin.order_order.order_sub_page —— 孪生生成器产物(请求面;行为面归场景用例)。
...
溯源统计: frontend=53, lang=28, derived=12, column=7, builtin=4, 无zh=1 | fe_high=68, enum=1
"""
...
ORDER_ORDER_SUB_PAGE: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.order_sub_page',
    system='fin',
    service='fin-service',
    name='Order.orderSubPage',
    description='Order.orderSubPage' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/orderSubPage',
        headers={},
        consumes=[],
        produces=[],
    ),
