"""fin.order_self.batch_update_supplier —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): order_self_id, service_project
溯源统计: column=4, lang=2 | fe_high=0, enum=2
"""
...
ORDER_SELF_BATCH_UPDATE_SUPPLIER: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_self.batch_update_supplier',
    system='fin',
    service='fin-service',
    name='操作',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/orderSelf/batchUpdateSupplier',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='isset_supplier', path=f'$.isset_supplier', type='integer', state='form', ui_kind='select', required=True, default='0', description='是否无供应商', enum=['0', '1']),
