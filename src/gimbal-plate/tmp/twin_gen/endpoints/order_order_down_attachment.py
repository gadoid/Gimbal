"""fin.order_order.down_attachment —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_DOWN_ATTACHMENT: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.down_attachment',
    system='fin',
    service='fin-service',
    ...
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='attachment_id', path=f'$.attachment_id', type='integer', state='form', ui_kind='number', required=True),
        ],
    ),
    responses={
        200: ResponseSpec(
