"""fin.pay_invoice_batch.apply_export —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): pay_invoice_apply_no, pay_invoice_batch_no, create_time, cancel_status, link_audit_status, customer_id, main_id, pay_settle_object_id, currency, invoice_status, invoice_no, writeoff_status, create_id, receive_invoice_batch_no, receive_invoice_apply_no, put_settle_object_id
溯源统计: frontend=19, lang=5, 无zh=5, builtin=2, column=1 | fe_high=24, enum=0
"""
...
PAY_INVOICE_BATCH_APPLY_EXPORT: Final[EndpointSpec] = EndpointSpec(
    id='fin.pay_invoice_batch.apply_export',
    system='fin',
    service='fin-service',
    name='PayInvoiceBatch.applyExport',
    description='PayInvoiceBatch.applyExport' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/payInvoiceBatch/applyExport',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='invoice_time_start', path=f'$.invoice_time_start', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='invoice_time_end', path=f'$.invoice_time_end', type='string', state='form', ui_kind='text'),
