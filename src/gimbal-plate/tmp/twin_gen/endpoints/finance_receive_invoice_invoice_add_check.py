...
needs_capture(首跑经 gimbal 执行回填): invoice_number, invoice_type, invoice_amount, invoice_date, currency, invoice_exchange_rate, invoice_original, seller_identity, put_settle_object, main_name, fee_main_name, receive_settle_object, sorce, replace_invoice_original, cancel_by, invoice_image_name
溯源统计: column=34, rule=13, 无zh=7 | fe_high=0, enum=2
"""
...
RECEIVE_INVOICE_INVOICE_ADD_CHECK: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='buyer_identity', path=f'$.buyer_identity', type='string', state='form', ui_kind='text', required=True),
            ...
            DeclarationEntry(name='put_settle_object', path=f'$.put_settle_object', type='string', state='form', ui_kind='text', description='应收结算对象'),  # zh_ambiguous
            DeclarationEntry(name='main_name', path=f'$.main_name', type='string', state='form', ui_kind='text', description='费用主体'),  # zh_ambiguous
            DeclarationEntry(name='invoice_apply_type', path=f'$.invoice_apply_type', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='receive_invoice_id', path=f'$.receive_invoice_id', type='integer', state='carry', ui_kind='number'),
