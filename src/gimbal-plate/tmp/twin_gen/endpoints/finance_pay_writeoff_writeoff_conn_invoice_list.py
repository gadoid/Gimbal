"""fin.pay_writeoff.writeoff_conn_invoice_list —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): page_no, page_size, sort_field, sort_order
溯源统计: builtin=4, column=1 | fe_high=0, enum=1
"""
...
PAY_WRITEOFF_WRITEOFF_CONN_INVOICE_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.pay_writeoff.writeoff_conn_invoice_list',
    system='fin',
    service='fin-service',
    name='PayWriteoff.writeoffConnInvoiceList',
    description='PayWriteoff.writeoffConnInvoiceList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/payWriteoff/writeoffConnInvoiceList',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='pay_writeoff_id', path=f'$.pay_writeoff_id', type='string', state='form', ui_kind='text', required=True, description='核销ID'),
