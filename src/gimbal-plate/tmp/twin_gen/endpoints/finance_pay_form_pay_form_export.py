"""fin.pay_form.pay_form_export —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): create_time, customer_id, customer_main_id, main_id, pay_settle_object_id, currency, status, writeoff_status, create_id, pay_invoice_apply_no, pay_invoice_batch_no, cancel_status, link_audit_status, invoice_status, invoice_no, receive_invoice_apply_no, receive_invoice_batch_no, put_settle_object_id
溯源统计: frontend=26, lang=6, derived=6, 无zh=4, column=2, builtin=1 | fe_high=31, enum=1
"""
...
PAY_FORM_PAY_FORM_EXPORT: Final[EndpointSpec] = EndpointSpec(
    id='fin.pay_form.pay_form_export',
    system='fin',
    service='fin-service',
    name='PayForm.payFormExport',
    description='PayForm.payFormExport' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/payForm/payFormExport',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='pay_form_ids', path=f'$.pay_form_ids', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='check_ids', path=f'$.check_ids', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='real_total_start', path=f'$.real_total_start', type='string', state='form', ui_kind='text', description='金额开始'),
            DeclarationEntry(name='real_total_end', path=f'$.real_total_end', type='string', state='form', ui_kind='text', description='金额结束'),
            DeclarationEntry(name='pay_settle_object_receive_account', path=f'$.pay_settle_object_receive_account', type='string', state='form', ui_kind='text', description='收款账户'),
            ...
            DeclarationEntry(name='sort_order', path=f'$.sort_order', type='string', state='form', ui_kind='select', default='desc', description='排序方向', enum=['asc', 'desc']),
        ],
    ),
    responses={
        200: ResponseSpec(
