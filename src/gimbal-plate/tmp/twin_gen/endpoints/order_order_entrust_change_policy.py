"""fin.order_entrust.change_policy —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_ENTRUST_CHANGE_POLICY: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_entrust.change_policy',
    system='fin',
    service='fin-service',
    name='操作',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/orderEntrust/changePolicy',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='policy_id', path=f'$.policy_id', type='integer', state='form', ui_kind='number', required=True, description='服务策略'),
