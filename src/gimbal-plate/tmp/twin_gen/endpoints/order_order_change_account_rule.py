"""fin.order.change_account_rule —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_CHANGE_ACCOUNT_RULE: Final[EndpointSpec] = EndpointSpec(
    id='fin.order.change_account_rule',
    system='fin',
    service='fin-service',
    name='操作目的',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/changeAccountRule',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='supplier_period', path=f'$.supplier_period', type='integer', state='form', default='0', description='供应商账期'),
