"""fin.audit.audit_add —— 孪生生成器产物(请求面;行为面归场景用例)。
...
溯源统计: column=19, 无zh=3 | fe_high=0, enum=0
"""
...
AUDIT_AUDIT_ADD: Final[EndpointSpec] = EndpointSpec(
    id='fin.audit.audit_add',
    system='fin',
    service='fin-service',
    name='Audit.auditAdd',
    description='Audit.auditAdd' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/home/audit/auditAdd',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='audit_id', path=f'$.audit_id', type='integer', state='carry', ui_kind='number'),
