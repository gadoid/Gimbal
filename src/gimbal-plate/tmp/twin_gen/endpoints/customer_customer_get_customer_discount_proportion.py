...
needs_capture(首跑经 gimbal 执行回填): customer_id, trade_term, airline, volume
溯源统计: column=3, lang=3 | fe_high=0, enum=0
"""
...
CUSTOMER_GET_CUSTOMER_DISCOUNT_PROPORTION: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='trade_term', path=f'$.trade_term', type='string', state='form', ui_kind='text', description='成交方式'),  # zh_ambiguous
            DeclarationEntry(name='airline', path=f'$.airline', type='string', state='form', ui_kind='text', description='航线'),  # zh_ambiguous
            DeclarationEntry(name='volume', path=f'$.volume', type='string', state='form', ui_kind='text', description='箱型'),  # zh_ambiguous
            DeclarationEntry(name='atd', path=f'$.atd', type='integer', state='form', ui_kind='number', default='0', description='实际开航日'),
            DeclarationEntry(name='order_id', path=f'$.order_id', type='integer', state='form', ui_kind='number', default='0', description='业务订单ID'),
        ],
    ),
    responses={
        200: ResponseSpec(
