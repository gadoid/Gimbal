"""fin.pay_form.form_detail —— 孪生生成器产物(请求面;行为面归场景用例)。
...
PAY_FORM_FORM_DETAIL: Final[EndpointSpec] = EndpointSpec(
    id='fin.pay_form.form_detail',
    system='fin',
    service='fin-service',
    name='操作',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/payForm/formDetail',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='pay_form_id', path=f'$.pay_form_id', type='string', state='form', ui_kind='text', description='付款单ID'),
