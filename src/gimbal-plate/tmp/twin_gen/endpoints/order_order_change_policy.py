"""fin.order_order.change_policy —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_CHANGE_POLICY: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.change_policy',
    system='fin',
    service='fin-service',
    name='操作',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/changePolicy',
        headers={},
        consumes=[],
        produces=[],
    ),
