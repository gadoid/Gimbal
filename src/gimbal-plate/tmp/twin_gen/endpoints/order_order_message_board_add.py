...
ORDER_MESSAGE_BOARD_ADD: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.message_board_add',
    system='fin',
    service='fin-service',
    name='Order.messageBoardAdd',
    description='Order.messageBoardAdd' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/messageBoardAdd',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='relation_id', path=f'$.relation_id', type='integer', state='form', ui_kind='number', required=True),
            DeclarationEntry(name='type', path=f'$.type', type='string', state='carry', ui_kind='text', required=True, description='留言类型'),  # needs_capture:value_source
            DeclarationEntry(name='message', path=f'$.message', type='string', state='form', ui_kind='text', required=True, description='留言内容'),
            DeclarationEntry(name='user_ids', path=f'$.user_ids', type='string', state='form', ui_kind='text', required=True, description='通知人ID'),
