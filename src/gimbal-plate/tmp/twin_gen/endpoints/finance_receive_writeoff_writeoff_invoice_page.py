...
溯源统计: frontend=26, 无zh=7, derived=6, column=5, builtin=4, lang=1 | fe_high=27, enum=1
"""
...
RECEIVE_WRITEOFF_WRITEOFF_INVOICE_PAGE: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='receive_invoice_ids', path=f'$.receive_invoice_ids', type='string', state='form', ui_kind='text', required=True),
            DeclarationEntry(name='fee_main_id', path=f'$.fee_main_id', type='integer', state='form', ui_kind='select', default='0', description='费用主体'),
            ...
            DeclarationEntry(name='invoice_amount_start', path=f'$.invoice_amount_start', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='invoice_amount_end', path=f'$.invoice_amount_end', type='string', state='form', ui_kind='text'),
            ...
            DeclarationEntry(name='usd_amount_start', path=f'$.usd_amount_start', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='usd_amount_end', path=f'$.usd_amount_end', type='string', state='form', ui_kind='text'),
            DeclarationEntry(name='bl_no', path=f'$.bl_no', type='string', state='form', ui_kind='text', description='提单号'),  # zh_ambiguous
            DeclarationEntry(name='is_replace', path=f'$.is_replace', type='integer', state='form', ui_kind='number', default='0', description='是否被替换 1是'),
