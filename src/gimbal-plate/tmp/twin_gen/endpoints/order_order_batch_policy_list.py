"""fin.order_order.batch_policy_list —— 孪生生成器产物(请求面;行为面归场景用例)。
...
ORDER_BATCH_POLICY_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.order_order.batch_policy_list',
    system='fin',
    service='fin-service',
    name='Order.batchPolicyList',
    description='Order.batchPolicyList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/order/order/batchPolicyList',
        headers={},
        consumes=[],
        produces=[],
    ),
