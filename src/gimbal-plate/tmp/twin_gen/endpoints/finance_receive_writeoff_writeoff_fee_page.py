"""fin.receive_writeoff.writeoff_fee_page —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): order_fee_real_ids, symbol, sort_field, sort_order
溯源统计: builtin=4, 无zh=2, column=1 | fe_high=0, enum=1
"""
...
RECEIVE_WRITEOFF_WRITEOFF_FEE_PAGE: Final[EndpointSpec] = EndpointSpec(
    id='fin.receive_writeoff.writeoff_fee_page',
    system='fin',
    service='fin-service',
    name='ReceiveWriteoff.writeoffFeePage',
    description='ReceiveWriteoff.writeoffFeePage' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/receiveWriteoff/writeoffFeePage',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='order_fee_real_ids', path=f'$.order_fee_real_ids', type='string', state='carry', ui_kind='text', required=True),  # needs_capture:value_source
