...
needs_capture(首跑经 gimbal 执行回填): receive_invoice_id, invoice_number, invoice_type, invoice_amount, invoice_date, currency, invoice_exchange_rate, bl_no, fee_main_name, receive_settle_object, sorce, replace_invoice_original, cancel_by
溯源统计: column=31, rule=16, 无zh=3, lang=1 | fe_high=0, enum=4
"""
...
RECEIVE_INVOICE_INVOICE_EDIT: Final[EndpointSpec] = EndpointSpec(
    ...
    request=RequestSpec(
        body_type='json',
        declarations=[
            DeclarationEntry(name='receive_invoice_id', path=f'$.receive_invoice_id', type='string', state='form', ui_kind='textarea', required=True, description='发票id'),  # zh_ambiguous
            ...
            DeclarationEntry(name='invoice_type', path=f'$.invoice_type', type='string', state='form', ui_kind='select', required=True, description='发票类型 1:增值税普通发票 2:增值税专用发票', enum=['1', '2']),  # zh_ambiguous
            DeclarationEntry(name='invoice_amount', path=f'$.invoice_amount', type='number', state='form', ui_kind='number', required=True, default='0.00', description='发票金额'),  # zh_ambiguous
            DeclarationEntry(name='invoice_tax_amount', path=f'$.invoice_tax_amount', type='number', state='form', ui_kind='number', required=True, default='0.00', description='发票税额'),
            DeclarationEntry(name='invoice_date', path=f'$.invoice_date', type='string', state='form', ui_kind='text', required=True, description='发票日期'),  # zh_ambiguous
            DeclarationEntry(name='currency', path=f'$.currency', type='string', state='form', ui_kind='select', required=True, description='发票币种', enum=['CNY', 'USD']),  # zh_ambiguous
            DeclarationEntry(name='usd_amount', path=f'$.usd_amount', type='number', state='form', ui_kind='number', default='0.00', description='美元金额'),
            DeclarationEntry(name='invoice_exchange_rate', path=f'$.invoice_exchange_rate', type='string', state='form', ui_kind='text', description='发票汇率'),  # zh_ambiguous
            ...
            DeclarationEntry(name='buyer_chinese_header', path=f'$.buyer_chinese_header', type='string', state='form', ui_kind='text', required=True, description='购买方中文抬头'),
            DeclarationEntry(name='buyer_identifier_no', path=f'$.buyer_identifier_no', type='string', state='form', ui_kind='text', required=True, description='购买方纳税人识别号'),
            DeclarationEntry(name='buyer_identity', path=f'$.buyer_identity', type='string', state='form', ui_kind='select', description='买方身份（可选，传入时按身份精确匹配应收结算对象）', enum=['customer', 'main']),
            DeclarationEntry(name='seller_chinese_header', path=f'$.seller_chinese_header', type='string', state='form', ui_kind='text', required=True, description='销售方中文抬头'),
            DeclarationEntry(name='seller_identifier_no', path=f'$.seller_identifier_no', type='string', state='form', ui_kind='text', required=True, description='销售方纳税人识别号'),
            DeclarationEntry(name='seller_identity', path=f'$.seller_identity', type='string', state='carry', ui_kind='select', description='卖方身份（可选，销售方须为系统内主体，费用主体随税号联动）', enum=['main']),
            DeclarationEntry(name='bl_no', path=f'$.bl_no', type='string', state='form', ui_kind='text', description='提单号'),  # zh_ambiguous
            DeclarationEntry(name='invoice_image_name', path=f'$.invoice_image_name', type='string', state='form', ui_kind='text', description='发票影像名称'),
            ...
            DeclarationEntry(name='receive_settle_object_id', path=f'$.receive_settle_object_id', type='integer', state='carry', ui_kind='number', description='应收结算对象ID'),
            DeclarationEntry(name='receive_settle_object', path=f'$.receive_settle_object', type='string', state='carry', ui_kind='text', description='应收结算对象'),  # needs_capture:value_source
            ...
            DeclarationEntry(name='generate_status', path=f'$.generate_status', type='integer', state='carry', ui_kind='number', description='0默认/1生成中/2已生成'),
