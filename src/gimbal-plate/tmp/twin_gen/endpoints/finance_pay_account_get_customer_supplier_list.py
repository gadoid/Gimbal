"""fin.pay_account.get_customer_supplier_list —— 孪生生成器产物(请求面;行为面归场景用例)。
...
PAY_ACCOUNT_GET_CUSTOMER_SUPPLIER_LIST: Final[EndpointSpec] = EndpointSpec(
    id='fin.pay_account.get_customer_supplier_list',
    system='fin',
    service='fin-service',
    name='PayAccount.getCustomerSupplierList',
    description='PayAccount.getCustomerSupplierList' + ' [generated:fin-test@2026-09-15]',
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/finance/payAccount/getCustomerSupplierList',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='target', path=f'$.target', type='string', state='form', ui_kind='text'),
