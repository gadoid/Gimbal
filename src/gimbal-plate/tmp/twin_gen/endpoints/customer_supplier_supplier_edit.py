"""fin.supplier.supplier_edit —— 孪生生成器产物(请求面;行为面归场景用例)。

来源: 代码生成 | 基线: fin-test@2026-09-15 | 生成时间: 2026-09-20T04:38:13+00:00
needs_capture(首跑经 gimbal 执行回填): supplier_id, status
溯源统计: column=24, 无zh=15, lang=4 | fe_high=0, enum=1
"""
...
SUPPLIER_SUPPLIER_EDIT: Final[EndpointSpec] = EndpointSpec(
    id='fin.supplier.supplier_edit',
    system='fin',
    service='fin-service',
    name='操作目的',
    ...
    api=ApiSpec(
        service='fin-service',
        method='POST',
        path='/api/customer/supplier/supplierEdit',
        headers={},
        consumes=[],
        produces=[],
    ),
    request=RequestSpec(
        ...
        declarations=[
            ...
            DeclarationEntry(name='supplier_no', path=f'$.supplier_no', type='string', state='carry', ui_kind='text', description='供应商编号'),
            DeclarationEntry(name='supplier_number', path=f'$.supplier_number', type='string', state='carry', ui_kind='text', description='供应商编号-填写'),
            ...
            DeclarationEntry(name='settlement_date', path=f'$.settlement_date', type='string', state='carry', ui_kind='text', description='结算业务供应商结算日'),
            DeclarationEntry(name='country_id_cn', path=f'$.country_id_cn', type='integer', state='carry', ui_kind='number'),
            DeclarationEntry(name='country_name_cn', path=f'$.country_name_cn', type='string', state='carry', ui_kind='text', description='中文'),
            DeclarationEntry(name='province_code', path=f'$.province_code', type='integer', state='carry', ui_kind='number'),
            DeclarationEntry(name='province_name', path=f'$.province_name', type='string', state='carry', ui_kind='text'),
            DeclarationEntry(name='city_code', path=f'$.city_code', type='integer', state='carry', ui_kind='number'),
            DeclarationEntry(name='city_name', path=f'$.city_name', type='string', state='carry', ui_kind='text'),
            DeclarationEntry(name='county_code', path=f'$.county_code', type='integer', state='carry', ui_kind='number'),
