...
**yaml 中 `Config.users` 与执行期注入的合并策略（当前默认 origin）**：

| 策略 | 行为 | 适用场景 |
|---|---|---|
| **A. 替换（override）** | 临时 yaml 的 `Config.users` 完全用"执行用认证"列表生成，原 yaml 的 `users` **整体丢弃** | 公共用例作者未编任何 `users`，由执行人全权注入；alias 必须全部可解 |
...
| **D. 不注入（origin，当前默认）** | 平台**完全不动** yaml 的 `Config.users`；跳过一次 DB 凭据解密往返。"执行用认证"字段被禁用并自动清空 | 用例 yaml 已经自带完备 `Config.users`，平台不必碰；最安全也最快 |

> 默认 D 选的理由：最常见的情况是作者在 yaml 里写好了 `Config.users`，让执行人手动注入反而容易把 yaml 里的"基线用户"覆盖掉。origin 模式下平台零介入，yaml 是什么就用什么。后端 schema 把 `merge_policy` 与 `inject_credentials` 两个字段独立处理 —— `inject_credentials=False` 时整个 Config.users 块跳过，前 3 个策略仅在 `inject_credentials=True` 时生效。
...
字段：
- **执行用认证**（多选，origin 模式下禁用）：下拉列出 `GET /api/auths` 当前用户的全部 self 凭证，按 alias 排序。每条形如 `codfish | https://fin-tidb... | Bearer`；可搜索过滤；
- **默认 token_type**（下拉，默认跟随第一个选中的认证，可下拉切换 `Bearer/Basic/Cookie/Custom Header`）：仅作用于"执行用认证"未指定的字段（V0.1 仅留底，UI 默认禁用，V1 解锁覆盖）；
- **合并策略**（单选，参考 §4.4 D3）：
  - `origin`（默认）：不注入 —— 平台不动 yaml 的 `Config.users`，下方"执行用认证"被禁用并自动清空；
  - `override`：原 yaml 的 `users` 整体替换为"执行用认证"列表；
...
1. **校验 alias 一致性**：扫描当前场景所有 `${auth.<alias>.token}` / `${auth.<alias>.*}` 引用 → `referenced_aliases`；与"执行用认证"列表比对 → 缺失集合缺失则直接拒绝（400 + D4 错误码）；多选但未引用的 alias 给出 warning，不阻断。origin 模式下跳过此校验（既不注入也不校验，yaml 自带的 alias 直接生效）。
2. **临时 YAML 落盘**：渲染当前 scenario 对象 → 按合并策略构造新的 `Config.users` → `yaml.safe_dump` 到 `data/tmp/exec_<executionId>_<idx>.yaml`（**注意磁盘上原 yaml 始终不动**）。origin 模式下 `Config.users` 完全跳过注入步骤，落盘 yaml 的 users 与原 yaml 一致。
...
| POST | `/executions` | 触发执行（body: `{case_id, n_runs, parallel, env, prefix?, exec_auth_alias?, merge_policy?, inject_credentials?}`，`inject_credentials` 默认 `true`；详见 §4.4 D3 / §4.5 E2） |
...
7. **`requirementRef`**：现行为 `list[str]`（需求管理系统的关联链接，纯信息字段），前端按"字符串数组"展示。
