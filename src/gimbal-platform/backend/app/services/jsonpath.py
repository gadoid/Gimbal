"""services/jsonpath.py(backend)

轻量级 JSON 路径解析器，零外部依赖。

本文件是 ``gimbal/utils/jsonpath.py`` 的镜像拷贝(与 ``gimbal_plate/utils/jsonpath.py``
同款先例;公开 API:get/get_all/set_value/delete/exists/resolve_template)——
改动三处同步;backend 与 gimbal/plate 互不引用此模块。
