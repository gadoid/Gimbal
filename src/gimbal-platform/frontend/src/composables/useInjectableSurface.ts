...
export function useInjectableSurface(
  ...
  /* ── 降级后的重试(有界退避)─────────────────────────────────────
   * 取数时机:宿主挂载 / 步骤面变化时的 `ensure()`,以及**降级后**本块排的退避
   * 重试 —— 被条目引用的端点取数失败后按 `RETRY_BACKOFF_MS` 三档各试一次,试满
   * 即停(有界)。没有它,挂载时刻的一次 plate 抖动就把判定面锁在从严姿态**整个
   * 会话**(悬空条目不可勾选),而失败本身对用户不可见。定时器落在组合式内 ——
   * 它是「取」,与 `ensure()` 同类,读路径仍不发请求(IS-7 不受影响)。
   * **触发面 ≠ 覆盖面**:触发只看 `degraded`(被条目引用的端点,与 `pending` 同面
   * ——否则与条目无关的端点失败会让判定面反复重取),每次重试取的却是**全部**步骤
   * 端点。故「只被画布需要、无条目引用」的那个端点失败时不会自动重试,由画布失败
   * 态上的手动入口兜住(两侧读同一份缓存)。 */
