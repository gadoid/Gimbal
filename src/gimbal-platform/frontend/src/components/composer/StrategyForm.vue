...
// immediate:focusJump 跨路由定位时 jumpSeq 先于策略卡挂载(kinds 拉取
// 晚到),卡生而 expandWhen=true — 无 immediate 则 watch 不触发永不展开
watch(() => props.expandWhen, (v) => { if (v) expanded.value = true }, { immediate: true })
