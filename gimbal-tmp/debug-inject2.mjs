...
await page.locator('.el-input__inner').first().fill('dbg-inject2')
...
await page.waitForTimeout(600)
// 检查 dialog 内部状态:确认按钮是否 disabled(alias 未选时 disabled)
const st = await page.evaluate(() => ({
  dialogTitle: document.querySelector('.el-dialog__title')?.textContent,
  confirmDisabled: [...document.querySelectorAll('.el-dialog__footer button')].map((b) => ({ t: b.textContent?.trim(), disabled: b.disabled })),
}))
console.log('dialog state:', JSON.stringify(st))
...
console.log('after select:', JSON.stringify(st2))
...
console.log('dialog still open:', await page.evaluate(() => !!document.querySelector('.el-dialog')))
await browser.close()
