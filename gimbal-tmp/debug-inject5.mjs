...
// addInitScript 是 context 级
await ctx.addInitScript(() => {
  window.__errs = []
  window.addEventListener('error', (e) => {
    window.__errs.push({ msg: String(e.message), stack: (e.error && e.error.stack) || '' })
  })
  // 未处理 promise rejection 也抓
  window.addEventListener('unhandledrejection', (e) => {
    window.__errs.push({ msg: 'rejection: ' + String(e.reason), stack: (e.reason && e.reason.stack) || '' })
  })
})
...
await page.locator('.el-input__inner').first().fill('dbg5')
...
for (const e of errs) console.log('ERR:', e.msg, '\n  STACK:', e.stack.split('\n').slice(0, 4).join('\n  '))
