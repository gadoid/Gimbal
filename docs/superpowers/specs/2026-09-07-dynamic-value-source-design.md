...
> 修订 9:2026-09-10 首连测实证(SUT 可达,§13.8 手验期)——**业务码信封检测 + 凭证复活钩子**:fin SUT 的凭证失效在业务码层应答(HTTP 200 + `code=401/407` + 空 data;407 msg="登录已过期"= 坏头/死 token 统一应答),HTTP 401 检测不可见 → 死凭证被当"合法空行集"静默缓存/供数,§7.5 降级态不可达。修复:§4.2 处理词表增"携 `code` 信封且非成功码 → 错误处理"(401/407 与 HTTP 401 同口径拉黑,其余业务码 sut_error;无信封响应不受影响)+ 凭证 patch/delete 路由挂 `drop_credential` 复活钩子(§7.5"到认证页刷新凭证"引导从此有真实效果;§6.2 不自动重登口径不变)。另实证 SUT token TTL ≈ 30min;**主根因(用户裁定)= 查询注入与认证部分的控制不一致**:runner 拼 `{token_type} {token}` scheme 头,而执行侧场景模板 = `${auth.<tag>.token}` 裸值(token_type 在执行链无消费)→ SUT 收 `Authorization <token>` 回业务码 407,platform 查询链路此前从未成功;修复 = runner 裸 token 注入同源对齐(§4.2/§13.8 留痕)。
> 修订 10:2026-09-10 多服务混编裁定(用户:"直接用当前 step 指定的服务,并使用对应的凭证查询")——查询身份 = **执行身份的域内首键**:`config.users` 中与当前 step 服务(`step.api.service` → 场景 services 声明 URL → origin)同域的条目,键序首命中;零命中 → null 诚实 422(不回退异域首键:错域 token 必被拒且 401/407 拉黑会污染该凭证的本域查询,§13.8 实证);服务 URL 未知 → 首键回退。§6.1 重写,§4.2③ 别名求值同步;后端零改动(调用方求值先例);单服务场景零行为变化。
> 修订 11:2026-09-10 同域多用户裁定(dataset spec §8.5 追问触发,用户:"现在一起做")——查询身份锚定 **step 实际引用的 auth tag**:headers `${auth.<tag>.…}` 引用首命中即查询别名(它就是该步执行身份的声明,查询视角 = 执行视角,构造消解恢复至每凭证粒度);无引用 → 域内首键 fallback(修订 10 行为,单用户零变化);悬空引用不回退猜测 → 别名照发,后端诚实 422「未找到查询凭证」(悬空徽章另有显形)。配套 §5.1/§5.2:L1 缓存键与单飞键从 view 扩为 **(view, 查询凭证)**((owner, alias);auth=none 视图凭证分量 = None 全局共享)——否则 A 先查、B 吃 A 的缓存,权限视角在缓存层串台;drop_credential 复活钩子同步逐凭证清 L1(重存凭证可能是不同 SUT 账号,旧行集可见性作废,他凭证不误伤)。
> 日期:2026-09-07
...
3. 服务 URL 解析复用**平台**服务绑定(公共设施,非执行链私产——两线共享地基,
   §0.3);凭证 = AuthSession 托管的查询凭证(§6.1);鉴权头 = **裸 token**
   (`Authorization: <token>`),与认证部分的控制一致 —— 执行侧场景模板
   `${auth.<tag>.token}` 同源;**不消费 token_type 拼 scheme**(该字段在
   执行链零消费,查询侧单边依赖它拼头即 §13.8 的 407 事故,修订 9);
   ...
   > 门内,内网语境接受;URL 求值源 = authored `config.services`,别名 = 当前
   > step headers `${auth.<tag>.…}` 引用首命中(修订 11:查询锚执行 auth tag,
   > §6.1)▸ `config.users` 中与该服务 URL 同域的键序首命中 fallback(修订 10:
   > 查询身份 = 执行身份的域内首键;修订 8 runSchemes 通道已移除),均缺 =
   > 诚实 422);
   > 集中式解析不设;
...
- **业务码信封检测**(修订 9):响应携 `code` 键且非成功码 → 错误处理
  **不缓存**——401/407 与 HTTP 401 同口径(清 token + 拉黑,§6.2),其余业务码
  sut_error 诚实透出;无信封响应(code 键缺席)不受影响。空结果分形的前提
  是响应宣告成功——死凭证在业务码层应答(HTTP 200 + `code=401/407` + 空 data)
  若不检测,会被当"合法空行集"静默缓存/供数;
...
- **键 = (view, 查询凭证)**(修订 11):view 的 params 是视图身份的一部分
  (固定预设),键天然完整;凭证分量 = (owner, alias),auth=none 视图为
  None(可见性与凭证无关,全局共享)——同视图不同凭证各自缓存/单飞,A 先查、
  B 不吃 A 的行集(权限视角不在缓存层串台;条目数 = 视图 × 活跃凭证,LRU
  上界仍 64,§5.3 有界);未来若准入检索词,该词不进键(§4.2)。
...
| L2 同视图单飞 | `dict[键, Lock]`(键 = §5.1 的 (view, 凭证);参数面再并点击期 params,§13.3) | 同视图同凭证并发合一发,其余等共享结果 |
| L3 凭证闸 | `dict[credential, Lock]` | 同凭证**最多 1 个在途 SUT 请求**(跨视图串行) |

锁序:**单飞锁外层 → 凭证闸内层;永不同时持两把单飞锁** → 无死锁。合计
~10 行。
...
> 修订 10(2026-09-10)再重写:修订 8 的"`config.users` 首键"在多服务混编
> (一场景多 step 各走不同服务)下会把异域首键凭证发给当前服务——错域
> token 必被 SUT 拒,且业务码 401/407 拉黑会污染首键凭证的**本域**查询
> (§13.8 实证)。用户裁定:查询身份 = 执行身份的**域内**首键。

查询身份 = **执行身份的域内首键**(修订 10,用户裁定):查询别名 = `config.users`
中与**当前 step 的服务**(`step.api.service` → 场景 services 声明 URL → URL
origin,前端求值)同域的条目,键序首命中。查询凭证是**平台配置**,与平台
登录用户无关、与场景执行配置无关。config.users 键与凭证池(auth_sessions)
同一命名空间;池无该别名 → 诚实 422「未找到查询凭证」。分形:

- **单服务场景零变化**:同域唯一命中即首键(修订 8 行为不变);
- **零命中 → null**:不回退异域首键——错域 token 必被拒,且 401/407 拉黑
  会污染该凭证的本域查询(§13.8 实证);null 由后端凭证闸诚实 422;
- **服务 URL 未知(声明缺失/非合法 URL)→ 首键回退**:服务侧 service_url
  422 先炸,别名求值无从校验,首键是唯一合理兜底;
- 锚定**当前 step** 而非场景级:混编下每步各用各的域内凭证(用户裁定
  原话:"直接用当前 step 指定的服务,并使用对应的凭证查询")。

> 修订 11(2026-09-10,同域多用户——dataset spec §8.5 追问触发):域内
> 首键在同域多用户时把所有查选钉在首键用户视角(为首键可见、为 B 不可见
> 的值会进 B 的列,构造消解退化为断言兜底)。别名求值再升级:**当前 step
> headers 的 `${auth.<tag>.…}` 引用首命中 → 该 tag 即查询别名**——headers
> 引用就是该步执行身份的声明,查询视角 = 执行视角,构造消解恢复至每凭证
> 粒度(为 B 的列查到的候选就是 B 能用的);无引用 → 域内首键 fallback,
> 单用户/无引用场景零变化。悬空引用不回退猜测:别名照发,后端诚实 422
> 「未找到查询凭证」(悬空另有徽章显形)。配套缓存键扩凭证(§5.1)——
> 别名规则与缓存键任缺其一,权限视角都会串台。
...
- **权限腐烂**:查询凭证(常偏高权限)可见的候选 ≠ 执行账号可用的值。处置:
  ...
  (修订 11:headers 带 auth 引用的 step 查询 = 执行凭证,此边界由构造消解
  ——查询用执行账号,钉的值执行时必然查得;fallback 域内首键路径仍适用
  本条原处置);
...
| 边界 | 语义 | 处置 |
|---|---|---|
| 权限腐烂(查询凭证≠执行凭证) | 高权限查询账号的候选,执行账号未必可用 | headers 锚 auth tag(修订 11)→ 构造消解;fallback 域内首键路径仍最小权限账号 + 断言暴露;不静态预测(§6.3) |
...
| ④ | platform 缓存 | TTL 惰性过期 / LRU 逐出 / MAX_ROWS 截断 / 空列表缓存与漂移不缓存分形 / 错误不写成新条目 / refresh 旁路 / stale 回退与 STALE_MAX_WINDOW / L1 键凭证分形(同视图异凭证各自缓存互不污染;auth=none 跨别名共享,修订 11)/ drop_credential 逐凭证清 L1(他凭证不误伤) |
| ⑤ | platform 并发 | 同视图并发单飞(1 上游请求);单飞键含凭证(同视图异凭证并发各一发,修订 11);L3 凭证闸互斥(登录/装载不与在途并发,§5.2 勘误口径——跨视图闸互斥与锁序冒烟未专测,已知缺口);连续失败熔断窗 |
| ⑥ | platform 凭证 | 401 → 降级提示不重登录;CurrentUser 鉴权门 |
| ⑦ | 前端 | enum 分支优先级(ui_kind=text + enum → select);折叠区同款;number enum 写值类型;选择器行渲染 + 行扇出写值 + 缺列跳过 + 本地过滤;N=1 单写;分组键 = group(缺省 view;同 view 双角色拆组互不覆写);查询别名锚 headers auth 引用首命中(同域双用户取引用 tag 非首键;悬空照发不猜,修订 11);降级态(认证页直达) |
...
| 8 | token_type 字段语义澄清(执行链零消费、查询链已不消费;仅通用 auth_header 拼接消费 —— 认证页 UI 说明或默认值收敛;§13.8 留痕 a) | 认证页下次触碰时顺车 |
...
- **缓存**:带 query_params 的视图**不进 L1**(§4.2/§5.1 检索词裁定同款:
  ...
  查询凭证, 点击期 params)(修订 11 键扩同步);熔断仍按 view;
  凭证闸/死集拉黑零改动。
...
> 首连测发现留痕(2026-09-10,SUT 可达,手验 2-4 执行期):手验推进至
> ②级(customer_part 携 customer_id 查询)时"无结果"——**三层根因链**:
> (a) **主根因(注入不一致,用户裁定)**:runner 鉴权头 = auth_header
> property 拼 `{token_type} {token}`,而执行侧场景模板(实证 exec dump)=
> `${auth.codfish.token}` **裸值** —— token_type 在执行链无消费,凭证里
> 填的 'Authorization' 从未碍事;查询侧单边拿它拼 scheme → SUT 收到
> `Authorization <token>` 回业务码 407"登录已过期" + 空 data。
> **platform 查询链路此前从未成功过**(独立重放脚本自带可通的头形,造成
> "链路曾通、后来坏"的假象);修复 = runner 裸 token 注入,与认证部分
> 的控制同源(修订 9);token_type 语义澄清票挂账 #8;
> (b) **已修 bug(修订 9)**:runner 只检 HTTP 状态码,407/401 + 空 data
> 被当"合法空行集"静默缓存/供数 → 业务码信封检测(401/407 同 HTTP 401
> 拉黑口径)+ drop_credential 复活钩子;
> (c) **参数形态悬而未决**:用户浏览器发字符串 "3",脚本重放 int 3 有
> 数据——SUT 数据模型全字符串 id,倾向字符串亦通,链路修复后随用户
> 重试定论。另实证:token TTL ≈ 30min;407=坏头/死 token 统一应答,
> 401 出现在被顶替/GC 后;test 按钮(认证页「测试」)本身是独立登录,
> 单会话 SUT 上会顶掉 runner 现役 token(§6.2 互踢窗的活体展示)。
