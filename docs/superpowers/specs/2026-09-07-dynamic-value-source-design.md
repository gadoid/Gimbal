...
> 修订 9:2026-09-10 首连测实证(SUT 可达,§13.8 手验期)——**业务码信封检测 + 凭证复活钩子**:fin SUT 的凭证失效在业务码层应答(HTTP 200 + `code=401` + 空 data),HTTP 401 检测不可见 → 死凭证被当"合法空行集"静默缓存/供数,§7.5 降级态不可达。修复:§4.2 处理词表增"携 `code` 信封且非成功码 → 错误处理"(401 与 HTTP 401 同口径拉黑,其余业务码 sut_error;无信封响应不受影响)+ 凭证 patch/delete 路由挂 `drop_credential` 复活钩子(§7.5"到认证页刷新凭证"引导从此有真实效果;§6.2 不自动重登口径不变)。另实证 SUT token TTL ≈ 30min。
> 日期:2026-09-07
...
- **业务码信封检测**(修订 9):响应携 `code` 键且非成功码 → 错误处理
  **不缓存**——401 与 HTTP 401 同口径(清 token + 拉黑,§6.2),其余业务码
  sut_error 诚实透出;无信封响应(code 键缺席)不受影响。空结果分形的前提
  是响应宣告成功——死凭证在业务码层应答(HTTP 200 + `code=401` + 空 data)
...
> ②级(customer_part 携 customer_id 查询)时"无结果"——双层原因:
> (a) **已证实 bug(已修,修订 9)**:SUT 凭证失效在业务码层应答(HTTP 200 +
> code=401/407 + 空 data;token TTL ≈ 30min 自然过期),runner 只检 HTTP
> 状态码 → 死凭证被当"合法空行集"静默缓存/供数,§7.5 降级态不可达;
> (b) **参数形态悬而未决**:用户浏览器发字符串 "3"(无结果),脚本重放
> int 3 有数据——SUT 数据模型全字符串 id,倾向字符串亦通,待活 token
> 复验后定论(backend 重启后首次查询产生新 token)。
