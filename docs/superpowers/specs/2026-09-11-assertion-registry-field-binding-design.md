...
- 期望通道从「exp_* 专用期望变量」整体迁移为「**断言管理注册表 + 绑定配置**」
- 数据集重定义为纯值映射;断言期望经**绑定配置**(第三对象)注入
- 场景(用例)纯化:只描述业务过程,不再承载测试数据结构

已实施且保持不变:段网格 / 段下拉单选 / 置顶基线行 / TSV / CSV 值链 / 行详情 / 死键提示 / 期望列跳编排器。**期望列本身随 exp_* 退役退场**(§2/§8 迁移)。
...
- 注册表 = 场景级资产(同场景多数据集共享);覆写 + 追加两形态都要
- **数据集与断言管理是两个独立组件,低耦合 — 唯一连接点 = 绑定配置(§5),倾向灵活配置**
- CSV v1 只导值
...
**家**:场景文档平台侧新节 `assertion_registry`(与 orchestration 同级,**不入 gimbal definition/config — 引擎不感知**)。编辑入口 = 编排器配置区「断言管理」节;数据集侧只读消费(经绑定配置,§5)。
...
- 手工建条目合法(无 anchor 条目仍可被绑定,联动建议缺席)
...
## 5. 三对象解耦:数据集 / 断言注册表 / 绑定配置(2026-09-11 用户裁定)

数据集与断言管理是**两个独立组件**,唯一连接点 = 绑定配置:

| 对象 | 形状 | 感知 |
|---|---|---|
| 数据集 | `{name, description, rows: [纯 var-dict]}` | **不知道断言存在**;CSV/TSV/复制零牵连 |
| 断言注册表 | 场景级条目表(§3) | **不知道数据集存在** |
| 绑定配置 | 场景侧 `assertion_bindings: { [datasetId]: [规则] }`,规则 = `{row, assertId, expected}` | 引用两侧 |

- **绑定配置与断言管理同家**(场景文档平台侧);数据集文档不存任何绑定信息
- **行保持纯 var-dict,零保留键** — 现有数据集 / CSV / TSV 粘贴彻底零影响
- **悬空检测(死绑定规则,死键同款)**:assertId ∉ 注册表 / datasetId 已删 / row 越界 → 软提示
- **灵活性**:绑定规则可整体停用(清空);**同一数据集多套断言方案、运行时选用** = v2 协议位(单数组升为命名方案表,结构不推翻)
- **行引用 = 索引**:数据集编辑器是唯一行编辑面,编辑时同步改写绑定;载入时越界检测兜底 — 与注册表条目引用步骤索引(§3)同款纪律

## 6. 数据集侧 — 双区(视图)

**变量区**(现状保持):config.vars 列宇宙 + 段下拉单选 + 置顶基线行 + 三态格 + TSV/CSV 值链。期望列(exp_*)随 §8 迁移退场。

**断言区**(新增)= **绑定配置的编辑视图**(join 三源渲染,写绑定配置):
...
- **一键迁移**(数据集编辑器 / 场景详情入口,手动触发):
  ...
  3. 数据集行 exp_* 键 → `assertion_bindings[datasetId]` 补规则 `{row, assertId, expected}`;行本身删 exp_* 键(回归纯值映射)
...
- 断言区 = 绑定配置编辑视图(§6);断言管理页反向视图「哪些数据集绑了此条目」(v1 轻量:条目详情列绑定计数)
- 互跳:注册表条目 ↔ 编排器断言卡;断言区列头 ↔ 同

## 10. v1 边界与协议位

协议位(v2+,只留位不实现):其他策略种类 / 行级覆盖字段默认期望 / catch-all 值 / **多套绑定方案(同一数据集不同断言集,运行时选用)** / 位置寻址(数据集侧定向未模板化位置)/ 一键模板化(数据集编辑器代改场景)/ 绑定入 CSV。

## 11. 任务切分建议(供 writing-plans)
...
| T4 | 绑定配置模型(`assertion_bindings`)+ 数据集断言区视图(三源 join + 行索引同步) | 前端 |
| T5 | 物化链绑定 patch(三源汇合)+ 旧 exp_* 路径迁移期闸 | backend |
