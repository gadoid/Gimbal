...
## 13. 阶段①② 终审遗留(计划 B 输入,2026-09-14)

阶段①② 已实施并通过终审(be0e528..ee2cc46,约束检查 5/5)。终审 triage 后以下项 DEFER 到阶段③④:

| 项 | 归属 | 内容 |
|---|---|---|
| 端点错误面测试 | ③ | 新 CRUD 端点的 404/403 分支无直接用例(与 RunDialog v2 错误面测试一并补) |
| 迁移丢弃同名存量 | ④前 | 名恰为「默认方案」的存量方案迁移时被 conflict-skip 且 payload 键清空 = 定义丢失;④ 下线迁移前评估「并入 default」替代 |
| 快建弹层失败回调化 | ③ | PUT 失败时乐观关闭丢用户输入(emit 契约需确认回调化) |
| 参数钳位直测 | ③ | 工作台参数区钳位边界(stepTo/nRuns/parallel)无直接组件测试,与真正拦截侧(运行参数面)测试一并补 |
| PG JSON→JSONB | PG 迁移时 | payload 列现为通用 JSON,PG 落 JSON 而非 JSONB(双方原生支持,迁移时评估) |
| 旧弹窗删 default 幽灵 | ③自然消失 | 旧 RunDialog 可对置顶 default「删除」→ 服务端重建 → 条目复活(不变量守住,纯过渡期 UX) |
