...
### 0.5 底层结构问题与解决对照(2026-09-01 评审轮钉点)

本设计是否"解决底层结构问题"的逐条判定 —— 不是换一种摆法:

| # | 结构问题 | 根因形态 | 归一化机制 | 判定 |
|---|---|---|---|---|
| 1 | 三轴并行,消费方各自拼装,无一处能答"这端点有哪些声明" | fields/carry/schema(+assertable_fields)四根轴 | declarations 单一承重存储;旧轴全部派生现算(§4.1) | 构造性解决 |
| 2 | 通道语义隐式 —— 字段属于哪个面取决于"在哪根轴上"(606be60 事故土壤) | 通道不是条目属性 | channel 一等属性;path 全清单唯一,D4 重叠结构性消亡(§3.2) | 构造性解决 |
| 3 | 双真源腐化(model 双真源刚退役) | 两个承重轴必然漂移 | 一根承重 + 派生轴**不存在于存储,想漂没地方漂**;schema_ 降伴随(B1) | 构造性解决 |
| 4 | 形态错配:15/17 流量提取物 vs settlement 模型优先(fields 手写重复模型) | 一根轴伺候两种形态 | 扁平清单是提取物自然形态(桥零改动);declare() 从模型吸收,description 双写消失(P2.5) | 解决,各有落点 |
| 5 | 值回流 plate(D2 后门) | CarryEntry 无值字段是类型形状的巧合 | B6 显式校验 + 物化层不补默认(606be60)—— 值边界两侧都被钉住 | 校验性解决 |

防腐化的核心原理:**可从 declarations 算出的一律现算、永不存储** —— 上一代腐化死于"两个都存"。
...
| 场景 | 路径 | 代价 |
|---|---|---|
| 前端要真类型渲染(B5 余项) | type 字段已在条目上,declare() 已在吸收;放宽 B5 必填 + 存量脚本补齐;旧线上键不动,前端读 declarations | 便宜 |
| Type C 收编(§11.6) | 加 schema_only 通道或按 binding 声明:闭合表加行、B7 词表扩项、golden re-baseline、前端差集同步 | 有价,路径清晰 |
| 查询串/头字段声明 | path 词表与 channel 词表均字符串/Literal 可扩,扁平清单不排斥 `$.query.x` 类前缀约定 | YAGNI,结构不预支也不堵死 |

**不扩散即扩展性**:执行链免疫(读线上派生键)、策略描述符隔离(另一契约面)、值/流语义留 platform —— 动任何一块,爆炸半径圈死在自身。
...
**schema_ 原有能力清点(降级零功能损失,2026-09-01 盘问确认)**:

| 能力 | 归一化后 |
|---|---|
| Type C 载体(schema 有、声明没有,前端差集渲染) | 原样 —— 内容与差集语义均不动,正式落点 |
| 结构展示(settlement 模型 schema 展示面) | 原样 |
| Rule B 校验(body_type 与 schema 存在性) | 原样(§5 两行"不变") |
| declare() 吸收源(P2 新增) | **只多不少** —— 方向单向(schema→声明),不反向改写 |

失去的仅是"承重真源"地位 —— 现状里本无任何东西从 schema_ 派生(fields 是手写的),故对其为零功能变化;与 declarations 的关系从"并行两轴"变为"一根承重 + 一个语义定义的伴随"。
