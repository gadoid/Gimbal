...
## 10. 挂账(2026-09-07 评审重写)

评审方法:按利率排序 —— 日计息者(核心卖点残缺)先还;内网低频语境下安全票低息可推迟;已有兜底机制覆盖的票销账。原 1-9 编号保留供追溯;新增 0 = 未登记悬账补登(仅存于 FieldStateSelect.vue 组件注释,既未进 M2 任务清单也未进本清单 —— 评审补录)。

| # | 挂账 | 裁定 | 依据 / 触发器 |
|---|---|---|---|
| 0 | **carry 字段找回(搜索框)** — §5.4 设计为定位手段,未进 §6-M2-3 任务清单 | **已实施(2026-09-07,commit 65740b4c)** | 高息日计:§1.3 逃生门单向残废 —— 行尾下拉只挂渲染行,carry 不进树(buildNode 剪除)→ carry→form 无 UI 路径;plate 共识 carry 字段(备注族)进不了任何场景表单 → [2026-09-07 spec](2026-09-07-field-recovery-and-export-threading-design.md) §2(已按该 spec 落地:FieldStateSearch 搜索框 + cascadeIncrements 级联) |
| 6 | export 场景解析态穿线 | **已实施(2026-09-07,commit 65740b4c)** | 唯一正确性含金量:composer 定面与运行时注入(carry_injection)已 join 增量,plate 导出面基准仍 entry.state(Step 模型剥 field_states)→ 导出产物面与配置面不一致 → 同上 spec §3(已按该 spec 落地:Step 收编 field_states + 导出面切 resolve_state 解析链,dispatch 基线零重钉) |
...
| 4 | 装饰词表(label/order/when/readonly/语境默认值) | 观望 | 纯加性零腐烂(map 升级),晚做不付利息;真实 UI 诉求出现时定价。#0 落地时搜索结果行写 field_states 同路径即天然留缝(语境默认值) |
| 5 | 响应面可配置 | 观望 | 单脸 + assertable 够用(断言按 path 挑);等真实噪音反馈再定价 |
| 7 | 值表同名 path 漂移告警 | **销账(2026-09-07)** | 双重既有机制覆盖:carry_store orphan 健康检测(bound − face 差集,§4.1(a) 切读后仍活)+ 每次发版 A/B 对拍纪律;残余价值仅"发版间隙主动告警",不值立项。销账动作:发版 checklist 加一行"看 orphan 检测结果" |
| 8 | carry 空转检测 | **销账(2026-09-07)** | fail-closed 设计下空转无伤害(无值不注入),纯卫生指标不配立项;真需要时 carry_store 健康检查多吐一个数字即可 |
| 9 | plate 存储形态升级(文件→可写层) | **销账(2026-09-07)** | 触发条件是"默认修正频率上升",发动机是 L2;L2 冰冻 → 永不触发。内网低频下发版成本(spec §2.1 定价)继续有效 |
