...
每页迁移的验收动作:视觉走查对照本方案末尾对应原型图、该页面原有单测迁移到新组件结构后必须全绿、**手动过一遍该页面的完整交互路径**(不只是渲染,包括表单提交 / 弹窗 / 分页),按既有手验清单纪律记录。当前仓库**没有 E2E 基础设施**(devDependencies 无 playwright / cypress,测试资产是 vitest,60+ 测试文件),本方案不新增 E2E 建设;若后续要上,单独立项,不挂在本方案验收标准上。
