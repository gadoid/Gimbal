...
>
> **实施进度(2026-09-17,随批次更新)**:Phase 0 ✅(9c93d21/1b6f347/49d084e)→ Phase 1 ✅(c7feecd 交叉切面 + 1352649 结构层 + 4a3747f 评审回应)→ 批次 0 ✅(6c072bb 数据集路由退役 + c5803db 删除能力承接 + **P1 补齐编辑能力**:双模式对话框改名/改行 + TSV/CSV 粘贴 + CSV 导出,编辑保存原样带回 varUnlocks — 全量评审发现批次 0 曾漏编辑,违背 D1"维护能力承接"承诺,已修复并沉淀审计教训:**退役页面独占的能力不能只查调用方断链,要对照被删文件的能力清单**)→ 批次 1 ✅(c00bbc7 + f5326c9 Enter 双发修复)→ 表单范式 A 落地 ✅(b4af2c9 + Login 统一组合式)→ **批次 2 ✅**(54c37c0 用户管理 / 20f1fd9 传递字段 / Auths 认证管理,admin 三页 EP 退场;UsersAdmin/CarryConfig 首次补测试,Auths 状态机测试存活并扩至 6 条;el-select allow-create 场景由 Input+datalist 承接,记入范式注意项)→ **批次 3 ✅**(172a9bb 常量池 / 7dbfa46 适配中心含 ImpactDrawer+UnindexedAlert 随迁 / 批次详情视图层+OpPreview;ElMessageBox 五处直调清零 — 全部走 confirmAction)。**登记清零项**:OpConstructDialog(批次详情构造对话框,345 行动态表单)仍为 EP,Phase 3 前必须迁移 — Phase 3 门控(EP 零存活才可开始)兜底。→ **批次 4 ✅**(执行历史/详情迁移新栈;**状态色统一 Signal**(status-colors.css 与 token 同源,兑现 tailwind.config 批次 4 承诺);原型修订两项落地:时间列带日期(实测原实现已含)、失败数字红色可点 → 详情 ?rows=failed 自动展开行级表,+2 条钉子测试)→ **批次 5 ✅**(07423ba 场景详情枢纽页,实测 0 处 el-*:断言覆盖率徽标**新建**并整体可点直达注册表(徽标即入口,兑现 §一承诺;覆盖口径=含 assertion 策略的步骤/总步骤,三态 Signal 色);数据集计数可点直达方案工作台;「修改编排」→「编排」;原型注释文案实测不存在=no-op;本页首批 4 条测试)→ **批次 6 ✅**(e35b95b 断言注册表 / 54178be schemes 族+工作台 / 8de5435 共享组件 / 71407a8+ca72bb1 composer 步骤组件 / 收官片:CaseComposer 残留清零 + Scenarios 场景库 + OpConstructDialog 清零项兑现 — **全站业务面 el-* 归零**(仅 /dev/dual-stack 验证页保留,Phase 3 退场);ElMessageBox 全部走 confirmAction;两态收敛决策记录:未保存离开拦截的"放弃并离开"捷径取消(v2.2 修订定调))→ **§7 registry ✅**(59f3209:WorkbenchCardDef 契约 + 常量池首张注册卡 + WorkbenchCardSlot 故障隔离,7 条测试)→ **Phase 3 ✅**(EP 依赖从 package.json 移除 + main.ts 全局注册/全量 CSS 摘除 + override.css(--el-* 桥)退役 + preflight 复位 + /dev/dual-stack 退场 + store 死 action saveDataSet/removeDataSet 及其测试移除 + 38 个测试文件的 EP 插件挂载清理;build 8.8s→5.1s,bundle 收益兑现)。测试数 924 → 853 → 866:批次 0 退役约 120 条(随视图),新增约 90 条(toast/confirm/Sidebar/面包屑/App 三态/路由 meta/登录注册/数据区含编辑/表单),**非丢失**。全量评审遗留 P3 已清:退役数据集深链 redirect 兜底到方案工作台;场景改名后面包屑即时同步(useScenarioName 缓存改 reactive Map,composer 载入/保存两处 seed)。每笔提交点全量 vitest + vue-tsc + vite build 三绿。待办:**全路由侧边栏模式手动走查**(需后端在场,唯一验收遗留项)。代码侧全部完成:Phase 0-3 + §7 registry + 批次 0-6。(store 死 action `saveDataSet`/`removeDataSet`、`/dev/dual-stack` 退场、EP 依赖移除)。
...
**显式延后(不进本规范)**:~~拖拽自定义布局 / 卡片市场(v2)~~(2026-09-18 已落地:`layout.ts` 按用户分键 localStorage + AddCardGallery 卡片市场 + vuedraggable 把手拖排 + 卡槽 hover 移除,新卡:最近执行 limit 5/收藏场景 top5)、卡片间联动(MVP 明确禁止)、全局搜索 ⌘K(独立入口,另立项)、服务端布局同步。
