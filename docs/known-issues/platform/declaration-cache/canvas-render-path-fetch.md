...
2. **不要**据此推翻判定面的读 / 取分离约定：该纪律的适用面**已从「判定面」扩到
   「判定面 + 画布」** —— 画布已按本计划的 T7 收口，渲染期同样零请求（见文末
   [修复记录](#修复记录)）。本列表整体是**修复前**的读者指引：其第 1 条的
   「两个 computed 内部取数」与第 3 条的「收口方向」均已由该修复落实，逐条以
   修复记录为准；
...
### 1b. 覆盖面只解决**成功路径** —— 失败路径的自愈通道同时消失

上面的论证只说明「端点会被取」，不说明「取不到时会再试」。删除前，被删的两处
在**成功路径**上确实冗余，但在**失败路径**上是画布**唯一**的自愈通道：模板直调的
`fieldBindings(currentStep)` 每次重渲染都落到 `stepDecls`，所以挂载期抖动一次之后，
只要负缓存窗（`FAILED_RETRY_MS`，10s）过去，用户任何交互引起的重渲染都会重取 ⇒ 自愈。

删除后画布只剩两个取数点（预拉、目录加入 `onAddEndpoint`），两者都不是「重试」：
挂载期一次失败 ⇒ 该 step 的目录树**整个会话为空**（`:277` 的失败提示只是静态文字）。
后果与 `platform/declaration-cache/no-retry-after-degradation.md`（同型，**P1 未修复**）
一致。

**本波不在画布内补重试**（那正是本条要收口的东西），该通道**由本计划的 T8
（降级重试通道：有界退避 + 失败态可见）承接**，且 T8 的重试通道须覆盖画布、不只判定面。
⇒ **不要**从 §1 的覆盖面论证推出「失败也能自愈」。
...
**关闭范围 = 「渲染期取数」这一条**（读 / 取分离在画布上落地）。§1b 记的失败路径
无重试通道**不随本条关闭** —— 那是 `no-retry-after-degradation.md` 的既有 P1，
由本计划 T8 承接。
