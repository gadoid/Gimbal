...
```python
# src/gimbal-platform/backend/app/services/query_view_cache.py:54-60
def put(self, key, payload, fetched_wall, truncated=False) -> None:
...
```python
# :41-52  lookup：TTL 命中 → fresh；TTL 过但 ≤ stale_window → 回退候选；再久 → 真过期
```

于是当条目数超过 `DECLARED_PATHS_MAX_ENTRIES`（默认 **256**，`backend/app/core/config.py:98`）时，**最久未用**的条目会被逐出 —— 哪怕它仍在回退窗内（默认 **3600s**，`config.py:99`），本来还「可被回退服务」。
...
2. **256 的判据**：声明面是**端点级**粒度（一个 endpoint 一条）而非字段级/行级 —— 单个部署上「同时活跃的端点」数量远低于 256（`config.py:93-99` 把三个值一并裁定并注明「对照 query_view_runner 的 stale_max_window = 86400s，本值保守」）；
...
2. 调大 `DECLARED_PATHS_MAX_ENTRIES` 时记住代价是内存（条目载荷 = `(decls, frozenset(catalog_paths(decls)))`，见 `endpoint_declarations.py:218`）；该值来自 settings，`_cache()` 会比对新旧 cfg 并**换实例**（`endpoint_declarations.py:95-109`），故热改即刻生效（但**旧缓存整份丢弃** —— 换实例不是迁移）；
3. 若维护者要动这块，**先读完 `query_view_cache.py:1-13` 的载荷泛化说明**：`TtlLruCache` 由两个消费者共用（query-view 行集 + 声明面），改逐出策略会同时影响另一条链（`query_view_runner`）。
...
2. **共享缓存新增第三个消费者**：三个消费者共用一套容量/回退语义，逐出策略的影响面会再次扩大；
