...
> | [platform/declaration-cache/canvas-render-path-fetch.md](platform/declaration-cache/canvas-render-path-fetch.md) | platform/frontend | P2 | 未修复（**阶段二**） | 画布 `stepDecls` / `currentFull` 两个 computed 内部 `void ensureEndpointFull` ⇒ 渲染期取数：「读函数不得取数」只在判定面成立（缓存 + 负缓存兜底，不自维持） |
