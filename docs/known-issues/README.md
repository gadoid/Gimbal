...
> | [platform/declaration-cache/full-prefetch-on-light-pages.md](platform/declaration-cache/full-prefetch-on-light-pages.md) | platform/frontend | P2 | 已接受局限 | 轻量页也预取「每步的 `/full`」：**成功端点**每 300s 窗口 ≤1 次（TTL 未过期 + 在飞收敛）；失败端点另受有界退避，见 `no-retry-after-degradation.md` |
...
> | [platform/registry/candidate-dropdown-duplication.md](platform/registry/candidate-dropdown-duplication.md) | platform/frontend | P2 | **已评估，不做** | 编辑器（`JsonPathInput` 分层选择器）与画布（`FieldForm` 扁平候选列表）的候选输入不统一是**有意差异**；统一会删掉按层提示 |
...
> | [platform/judgment-face/stale-snapshot-test-latent-green.md](platform/judgment-face/stale-snapshot-test-latent-green.md) | platform/backend/tests | P2 | **已修复**（阶段二 Task 1） | 原 latent-green 已消解：用例迁到 `test_plate_full_cache.py` 并改为断言副作用（`stale` / `status` / `reason`）—— 冻时钟实测变红，判别力不再依赖时钟粒度 |
...
> | [platform/declaration-cache/no-retry-after-degradation.md](platform/declaration-cache/no-retry-after-degradation.md) | platform/frontend | **P1** | **已修复**（阶段二 Task 8） | 降级后有界退避自动重试（`[10s,30s,60s]` 三档、试满即停、覆盖面 = 全部步骤端点）+ 编辑器 / 画布 / 运行对话框三处失败态可见且带手动重试入口；**退避试满后仍需手动点**（见记录 §3 的残余） |
...
> | [platform/declaration-cache/canvas-render-path-fetch.md](platform/declaration-cache/canvas-render-path-fetch.md) | platform/frontend | P2 | **已修复**（阶段二 Task 7） | 画布两处渲染期取数（`stepDecls` 是函数、`currentFull` 是 computed）已收口：删两处 + 按 step 端点集的 `immediate: true` 预拉覆盖面加测试钉住 |
