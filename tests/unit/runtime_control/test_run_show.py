...
def test_step_map_strategy_kinds_are_normalized():
    ...
    assert payload["steps"][1]["strategy_kinds"] == ["assertion"]
    # 第三步无 strategy：缺字段或空 list 都接受
    assert "strategy_kinds" not in payload["steps"][2] or payload["steps"][2]["strategy_kinds"] == []
